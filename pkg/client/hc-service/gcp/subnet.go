...
// Update subnet.
func (v *SubnetClient) Update(ctx context.Context, h http.Header, id string, req *types.GcpSubnetUpdateOption) error {
	resp := new(rest.BaseResp)

	err := v.client.Patch().
		WithContext(ctx).
		Body(req).
		SubResourcef("/vendors/%s/subnets/%s", enumor.Gcp, id).
		WithHeaders(h).
		Do().
		Into(resp)
	if err != nil {
