...
func (cli *client) RouteTable(kt *kit.Kit, params *SyncBaseParams, opt *SyncRouteTableOption) (*SyncResult, error) {
	...
	addSlice, updateMap, delCloudIDs := common.Diff[typesroutetable.TCloudRouteTable,
		routetable.TCloudRouteTable](routeTableFromCloud, routeTableFromDB, isRouteTableChange)
	...
	// 更新子网的路由表信息
	if len(subnetMap) > 0 {
		err = common.UpdateSubnetRouteTableByIDs(kt, enumor.TCloud, subnetMap, cli.dbCli)
		if err != nil {
...
func (cli *client) syncRoute(kt *kit.Kit, params *SyncBaseParams,
	routeTableFromCloud []typesroutetable.TCloudRouteTable) error {
	...
	req := &hcroutetable.TCloudRouteTableSyncReq{
	...
	for _, one := range routeTableFromDB {
		for _, cloudOne := range routeTableFromCloud {
			if one.CloudID == cloudOne.CloudID {
				...
				err = routetablesync.TCloudRouteSync(kt, req, one.ID, converter.ValToPtr(cloudOne), cli.dbCli)
				if err != nil {
...
func (cli *client) createRouteTable(kt *kit.Kit, accountID string, resGroupName string,
	...
	subnetMap := make(map[string]dataproto.RouteTableSubnetReq, 0)
	createResources := make([]dataproto.RouteTableCreateReq[dataproto.TCloudRouteTableCreateExt], 0, len(addSlice))
	...
	_, err := cli.dbCli.TCloud.RouteTable.BatchCreate(kt.Ctx, kt.Header(), createReq)
	if err != nil {
		logs.Errorf("[%s] routetable batch compare db create failed. accountID: %s, resGroupName: %s, err: %v",
			enumor.TCloud, accountID, resGroupName, err)
		return subnetMap, err
	}

	logs.Infof("[%s] sync routeTable to create routeTable success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) updateRouteTalbe(kt *kit.Kit, accountID string, resGroupName string,
	...
	logs.Infof("[%s] sync routeTable to update routeTable success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) deleteRouteTable(kt *kit.Kit, accountID string, region string, delCloudIDs []string) error {
	...
	if len(delRouteTableFromCloud) > 0 {
		logs.Errorf("[%s] validate routeTable not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloud, checkParams, len(delRouteTableFromCloud), kt.Rid)
	...
	logs.Infof("[%s] sync routeTable to delete routeTable success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) listRouteTableFromCloud(kt *kit.Kit, params *SyncBaseParams) ([]typesroutetable.TCloudRouteTable, error) {
	...
	if err != nil {
		...
		logs.Errorf("[%s] list routeTable from cloud failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloud, err,
...
func (cli *client) listRouteTableFromDB(kt *kit.Kit, params *SyncBaseParams) (
	...
	if err != nil {
		logs.Errorf("[%s] list routeTable from db failed, err: %v, account: %s, req: %v, rid: %s", enumor.TCloud, err,
			params.AccountID, req, kt.Rid)
		return nil, err
	}

	routeTables := make([]routetable.TCloudRouteTable, 0)
	for _, one := range results {
		routeTables = append(routeTables, routetable.TCloudRouteTable(*one))
	}
...
func (cli *client) RemoveRouteTableDeleteFromCloud(kt *kit.Kit, accountID string, region string) error {
	...
	for {
		resultFromDB, err := cli.dbCli.TCloud.RouteTable.ListRouteTableWithExt(kt.Ctx, kt.Header(), req)
		if err != nil {
			logs.Errorf("[%s] request dataservice to list routeTable failed, err: %v, req: %v, rid: %s", enumor.TCloud,
				err, req, kt.Rid)
			return err
		}
