...
func (a *ApplicationOfDeleteSecretKey) CheckReq() error {
	if len(a.secretID) == 0 {
...
func (a *ApplicationOfDeleteSecretKey) getSubAccountIDBySecret() (string, error) {
	...
	if len(result.Details) == 0 {
		return "", fmt.Errorf("sub account secret(id=%s) not found", a.secretID)
	}
...
func (a *ApplicationOfDeleteSecretKey) getSubAccountNameForDisplay() (string, error) {
	subAccountID, err := a.getSubAccountIDBySecret()
	if err != nil {
		return "", err
	}
