...
The system SHALL provide a PATCH endpoint at `/api/v1/woa/bizs/{bk_biz_id}/plans/resources/gpu/order/overwrite` that accepts an order_id and a new details list, validates preconditions, deletes existing sub-orders, and creates new sub-orders under the same main order.
...
- **GIVEN** a GPU demand main order with status other than `REJECT_ALL` (e.g., `INIT`, `PENDING`, `DONE`, `REJECT`, `TERMINATE`)
