...
// DeleteCosBucket ...
func (svc *cosSvc) DeleteCosBucket(cts *rest.Contexts) (any, error) {
	req := new(cloudserver.ResourceDeleteReq)
	if err := cts.DecodeInto(req); err != nil {
		logs.Errorf("delete cos bucket request decode failed, err: %v, rid: %s", err, cts.Kit.Rid)
	...
	if err := svc.authorizer.AuthorizeWithPerm(cts.Kit, authRes); err != nil {
		logs.Errorf("delete cos bucket auth failed, err: %v, account id: %s, rid: %s", err, req.AccountID, cts.Kit.Rid)
		return nil, err
	}
...
func (svc *cosSvc) deleteTCloudCosBucket(kt *kit.Kit, rawReq json.RawMessage) (any, error) {
