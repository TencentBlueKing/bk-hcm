...
func (cli *client) listAccountFromCloud(kt *kit.Kit, opt *SyncSubAccountOption) ([]account.HuaWeiAccount, error) {
	if err := opt.Validate(); err != nil {
		return nil, errf.NewFromErr(errf.InvalidParameter, err)
	}
...
func (cli *client) listAccountFromDB(kt *kit.Kit, opt *SyncSubAccountOption) (
	...
	for {
		...
		resp, err := cli.dbCli.HuaWei.SubAccount.ListExt(kt.Ctx, kt.Header(), req)
		if err != nil {
