...
| 时间过滤 | [rules/time-filtering.md](rules/time-filtering.md) |
| 高级特性（组合字段、ES查询） | [rules/advanced-features.md](rules/advanced-features.md) |
...
4. **时间过滤用 VarTime 系列**：推荐使用支持多粒度的 VarTime 系列函数
...
```go
// 使用 VarTimeRange（推荐）
expr.Rules = append(expr.Rules, tools.VarTimeRangeExpression("fee_date", dateRange))

// 使用 Month
expr.Rules = append(expr.Rules, tools.MonthExpression("stat_date", month))
```
