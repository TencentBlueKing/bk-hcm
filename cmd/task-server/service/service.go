...
import (
	...
	"hcm/cmd/task-server/service/async"
...
func NewService(sd serviced.ServiceDiscover) (*Service, error) {
	...
	// 启动异步任务
	taskSer := pasync.NewTaskServer(pasync.HcmTaskManager)
	err = taskSer.InitProducer(cc.TaskServer().Async)
	if err != nil {
		logs.Errorf("init async server producer failed, err: %v", err)
		return nil, nil
	}
	err = taskSer.InitConsumer(cc.TaskServer().Async)
	if err != nil {
		logs.Errorf("init async server consumer failed, err: %v", err)
		return nil, nil
	}
