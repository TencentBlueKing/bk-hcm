...
// GcpRegion defines region dao operations.
...
var _ GcpRegion = new(gcpRegionDao)

// gcpRegionDao region dao.
type gcpRegionDao struct {
	orm   orm.Interface
	idGen idgenerator.IDGenInterface
}

// NewGcpRegionDao create a region dao.
func NewGcpRegionDao(orm orm.Interface, idGen idgenerator.IDGenInterface) GcpRegion {
	return &gcpRegionDao{
		orm:   orm,
		idGen: idGen,
	}
...
func (v *gcpRegionDao) BatchCreateWithTx(kt *kit.Kit, tx *sqlx.Tx, models []region.GcpRegionTable) (
	...
	sql := fmt.Sprintf(`INSERT INTO %s (%s)	VALUES(%s)`, models[0].TableName(),
		region.GcpRegionColumns.ColumnExpr(), region.GcpRegionColumns.ColonNameExpr())
...
func (v *gcpRegionDao) Update(kt *kit.Kit, filterExpr *filter.Expression, model *region.GcpRegionTable) error {
	if filterExpr == nil {
...
func (v *gcpRegionDao) List(kt *kit.Kit, opt *types.ListOption, whereOpts ...*filter.SQLWhereOption) (
	*typesRegion.GcpRegionListResult, error) {
	...
	tableName := table.GcpRegionTable
	if opt.Page.Count {
		...
		return &typesRegion.GcpRegionListResult{Count: count}, nil
	}
	...
	return &typesRegion.GcpRegionListResult{Details: details}, nil
}

// BatchDeleteWithTx batch delete region with transaction.
func (v *gcpRegionDao) BatchDeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, filterExpr *filter.Expression) error {
	if filterExpr == nil {
	...
	sql := fmt.Sprintf(`DELETE FROM %s %s`, table.GcpRegionTable, whereExpr)
