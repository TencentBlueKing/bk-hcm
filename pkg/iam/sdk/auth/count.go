...
func (a *Authorize) countIamIDKey(op operator.OpType, fv *operator.FieldValue) ([]string, error) {
...
func preAnalyzeContent(op operator.OpType, content *operator.Content) error {
...
func calculateSetForAnd(sets []iam.AuthorizeList, cnt int) (*iam.AuthorizeList, error) {
	...
	for _, base := range sets[idBase].Ids {
		...
		for _, set := range sets[1:] {
			// if this set's isAny is true skip it.
			if set.IsAny {
				continue
			}
			hit := false
			...
			if !hit {
				// one of the sets not hit, then all sets is not hit.
				hitOuter = false
				break
			}
