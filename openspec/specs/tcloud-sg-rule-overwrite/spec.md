# Spec: tcloud-sg-rule-overwrite

## Purpose

在资源下和业务下提供 TCloud 安全组规则全量覆盖能力。调用方可通过单次请求原子替换安全组的全部出站和入站规则，底层调用腾讯云 `ModifySecurityGroupPolicies`（不传 Version）保证云上操作原子性，成功后通过 `syncSGRule` 将本地 DB 规则与云上保持一致。当前仅支持 tcloud vendor。
