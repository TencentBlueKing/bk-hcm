...
func SyncAzureEip(kt *kit.Kit, req *SyncAzureEipOption,
	...
	for {
		...
		if len(tmpCloudIDs) > 0 {
			tmpIDs, tmpMap, err := getAzureEipDSSync(kt, tmpCloudIDs, req, dataCli)
			if err != nil {
	...
	dsIDs, err := getAzureEipAllDS(kt, req, dataCli)
	if err != nil {
		logs.Errorf("request getAzureEipAllDS failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
...
func syncAzureEipAdd(kt *kit.Kit, addIDs []string, req *SyncAzureEipOption,
	...
	for _, id := range addIDs {
		eip := &dataproto.EipExtCreateReq[dataproto.AzureEipExtensionCreateReq]{
			...
			Extension: &dataproto.AzureEipExtensionCreateReq{
				...
				SKUTier:           cloudMap[id].Eip.SKUTier,
				Zones:             cloudMap[id].Eip.Zones,
	...
	if len(createReq) > 0 {
		_, err := dataCli.Azure.BatchCreateEip(kt.Ctx, kt.Header(), &createReq)
		if err != nil {
			logs.Errorf("request dataservice to create azure eip failed, err: %v, rid: %s", err, kt.Rid)
			return err
		}
	}
...
func isAzureEipChange(db *AzureDSEipSync, cloud *AzureEipSync) bool {
	...
	if !assert.IsPtrStringEqual(cloud.Eip.SKUTier, db.Eip.Extension.SKUTier) {
		return true
	}

	if !assert.IsPtrStringSliceEqual(cloud.Eip.Zones, db.Eip.Extension.Zones) {
		return true
	}
...
func syncAzureEipUpdate(kt *kit.Kit, updateIDs []string, cloudMap map[string]*AzureEipSync,
	...
	if len(updateReq) > 0 {
		if _, err := dataCli.Azure.BatchUpdateEip(kt.Ctx, kt.Header(), &updateReq); err != nil {
...
func getAzureEipDSSync(kt *kit.Kit, cloudIDs []string, req *SyncAzureEipOption,
	...
	for {
		...
		results, err := dataCli.Azure.ListEip(kt.Ctx, kt.Header(), dataReq)
		if err != nil {
