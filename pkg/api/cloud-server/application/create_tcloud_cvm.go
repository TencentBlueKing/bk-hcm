...
var (
	passwordInvalidError = errors.New("the password must include 8-30 characters, " +
...
// Validate ...
// @param ignorePasswordVerification 某些情况下需要忽略密码校验，比如从DB获取的密码以加密，则需要解密后校验（这是复用TCloudCvmCreateReq结构导致的，后续可使用其他方式）
func (req *TCloudCvmCreateReq) Validate(ignorePasswordVerification bool) error {
...
func (req *TCloudCvmCreateReq) validatePassword() error {
	...
	if strings.ContainsAny(req.Password, punctuation) {
		satisfiedCount += 1
	}
