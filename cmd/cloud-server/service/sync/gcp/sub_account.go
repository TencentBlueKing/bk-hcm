...
func SyncSubAccount(kt *kit.Kit, cliSet *client.ClientSet, accountID string,
	...
	start := time.Now()
	logs.V(3).Infof("gcp account[%s] sync sub account start, time: %v, rid: %s", accountID, start, kt.Rid)
	...
	defer func() {
		logs.V(3).Infof("gcp account[%s] sync sub account end, cost: %v, rid: %s", accountID,
	...
	if err := cliSet.HCService().Gcp.Account.SyncSubAccount(kt, req); err != nil {
		logs.Errorf("sync gcp sub account failed, err: %v, req: %v, rid: %s", err, req, kt.Rid)
		return err
	}
