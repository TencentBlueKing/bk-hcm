...
func (rc *restClient) ListNetworkCvmRelWithExt(ctx context.Context, h http.Header,
	...
	[]*datarelproto.NetworkInterfaceExtWithCvmID[coreni.HuaWeiNIExtension], error) {

	resp := new(datarelproto.NetworkInterfaceCvmRelWithExtListResp[coreni.HuaWeiNIExtension])
	err := rc.client.Post().
		WithContext(ctx).
		Body(req).
