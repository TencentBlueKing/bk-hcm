...
// DeleteResPlanDemandGpuSubOrder delete res plan demand gpu sub order.
func (svc *service) DeleteResPlanDemandGpuSubOrder(cts *rest.Contexts) (interface{}, error) {
	...
	for {
		listResp, err := svc.dao.ResPlanDemandGpuSubOrder().List(cts.Kit, listOpt)
		if err != nil {
			logs.Errorf("delete list resource plan demand gpu sub order failed, err: %v, rid: %s", err, cts.Kit.Rid)
			return nil, fmt.Errorf("delete list resource plan demand gpu sub order failed, err: %v", err)
		}
	...
	if err != nil {
		logs.Errorf("delete resource plan demand gpu sub order failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return nil, err
	}
