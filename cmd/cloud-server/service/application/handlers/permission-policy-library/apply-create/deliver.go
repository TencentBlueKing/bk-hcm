...
func (a *ApplicationOfApplyPermPolicyLibCreate) GenerateApplicationContent() interface{} {
	return a.Content
}

// Deliver executes the actual resource creation after approval.
func (a *ApplicationOfApplyPermPolicyLibCreate) Deliver() (enumor.ApplicationStatus, map[string]interface{}, error) {
	resp, err := a.ApplyCreate(a.Cts.Kit, a.Content.Vendor, a.Content.PolicyLibraryID, []string{a.Content.AccountID})
	if err != nil {
		logs.Errorf("deliver: apply create failed, accountID: %s, err: %v, rid: %s",
