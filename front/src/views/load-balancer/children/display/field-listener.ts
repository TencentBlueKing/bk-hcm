...
export class DisplayFieldListener {
  @Column('string', { name: '监听器名称', index: 0, minWidth: 250 })
  name: string;

  @Column('string', { name: '监听器ID', index: 0 })
  cloud_id: string;

  @Column('enum', { name: '协议', index: 0, option: LISTENER_PROTOCOL_NAME, minWidth: 80 })
  protocol: string;

  @Column('number', { name: '端口', index: 0, sort: true, minWidth: 80 })
  port: number;
  ...
  @Column('number', { name: 'RS数量', index: 0, minWidth: 80 })
  rs_num: number;

  @Column('number', { name: '域名数量', index: 0, minWidth: 80 })
  domain_num: number;

  @Column('number', { name: 'URL数量', index: 0, minWidth: 80 })
  url_num: number;
  ...
  // 负载均衡相关字段
  @Column('string', { name: '负载均衡VIP', minWidth: 120 })
  lb_vip: string;

  @Column('string', { name: '负载均衡ID', minWidth: 120 })
  lb_cloud_id: string;
  ...
  @Column('string', { name: '网络类型', minWidth: 90 })
