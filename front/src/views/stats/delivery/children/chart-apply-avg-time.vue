...
const fetchChartData = async () => {
  loading.value = true;
  const res = await http.post('/api/v1/woa/task/apply/analysis/average_time_consumption/overview', {
  ...
  chartInstance.setOption({
    dataset: {
      source: chartData.value.map((item: any) => [item.year_month, item.avg_duration_hours]),
    },
  });
  loading.value = false;
  nextTick(() => {
