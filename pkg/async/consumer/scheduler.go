...
type scheduler struct {
	...
	scheduledFlowFetcher uint
	canceledFlowFetcher  uint
}
...
func NewScheduler(bd backend.Backend, exec Executor, ld leader.Leader, opt *SchedulerOption) Scheduler {
	...
	return &scheduler{
		...
		executor:             exec,
		leader:               ld,
		scheduledFlowFetcher: opt.ScheduledFlowFetcher,
		canceledFlowFetcher:  opt.CanceledFlowFetcher,
	}
}
...
func (sch *scheduler) scheduledFlowWatcher() {
	// 初始化协程池
	pool := newWorkerPool(sch.scheduledFlowFetcher)
	...
	for {
		...
		if err != nil {
			logs.Errorf("scheduledFlowWatcher failed to list tenants, err: %v", err)
			sch.sp.ExceptionSleep()
			continue
		}
...
func (sch *scheduler) canceledFlowWatcher() {
	// 初始化协程池
	pool := newWorkerPool(sch.canceledFlowFetcher)
	...
	for {
		...
		err := distributeTenantTasks(pool)
		if err != nil {
			logs.Errorf("canceledFlowWatcher failed to list tenants, err: %v", err)
			sch.sp.NormalSleep()
			continue
		}
