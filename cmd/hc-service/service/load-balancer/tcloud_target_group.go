...
func (svc *clbSvc) BatchModifyTCloudListenerTargetsWeight(cts *rest.Contexts) (any, error) {
	...
	tcloudAdpt, err := svc.ad.TCloud(cts.Kit, req.AccountID)
	if err != nil {
		logs.Errorf("modify listener rs weight api failed, get account failed, err: %v, accountID: %s, rid: %s",
	...
	err = tcloudAdpt.ModifyTargetWeight(cts.Kit, rsOpt)
	if err != nil {
		logs.Errorf("modify listener rs weight api failed, err: %v, lbID: %s, newWeight: %d, rsOpt: %+v, rid: %s",
			err, lbID, req.NewRsWeight, rsOpt, cts.Kit.Rid)
		return nil, err
	}
