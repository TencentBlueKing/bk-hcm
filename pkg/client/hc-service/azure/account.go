...
// Check 联通性和云上字段匹配校验
func (a *AccountClient) Check(ctx context.Context, h http.Header, request *hsaccount.AzureAccountCheckReq) error {
...
func (a *AccountClient) GetBySecret(ctx context.Context, h http.Header,
	req *cloud.AzureSecret) (*cloud.AzureInfoBySecret, error) {

	resp := new(account.BySecretResp[cloud.AzureInfoBySecret])
	err := a.client.Post().
		WithContext(ctx).
		Body(req).
...
func (a *AccountClient) GetResCountBySecret(kt *kit.Kit, request *cloud.AzureAuthSecret) (*hsaccount.ResCount, error) {
