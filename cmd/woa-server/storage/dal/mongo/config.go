...
// NewConf new mongo connect config
func NewConf(mongoConf *cc.MongoDB) (*Config, error) {
	if mongoConf.MaxOpenConns == 0 {
		mongoConf.MaxOpenConns = DefaultMaxOpenConns
	}

	if mongoConf.MaxIdleConns > MaximumMaxOpenConns {
		mongoConf.MaxIdleConns = MaximumMaxOpenConns
	}

	if mongoConf.MaxIdleConns < MinimumMaxIdleOpenConns {
		mongoConf.MaxIdleConns = MinimumMaxIdleOpenConns
	}

	if mongoConf.SocketTimeoutSeconds == 0 {
		mongoConf.SocketTimeoutSeconds = DefaultSocketTimeout
	}

	if mongoConf.SocketTimeoutSeconds > MaximumSocketTimeout {
		mongoConf.SocketTimeoutSeconds = MaximumSocketTimeout
	}

	if mongoConf.SocketTimeoutSeconds < MinimumSocketTimeout {
		mongoConf.SocketTimeoutSeconds = MinimumSocketTimeout
	}

	if mongoConf.RsName == "" {
		return nil, errors.New("rsName not set")
	}

	config := &Config{
		Address:       mongoConf.Host,
		Port:          mongoConf.Port,
		User:          mongoConf.Usr,
		...
		SocketTimeout: mongoConf.SocketTimeoutSeconds,
	}
