...
func (v syncSubnetSvc) SyncAwsSubnet(cts *rest.Contexts) (interface{}, error) {
	req := new(hcproto.AwsResourceSyncReq)
	...
	resp, err := subnetlogic.AwsSubnetSync(cts.Kit, req, v.ad, v.dataCli)
	if err != nil {
		logs.Errorf("request to sync aws subnet logic failed, req: %+v, err: %v, rid: %s", req, err, cts.Kit.Rid)
		return nil, err
	}
