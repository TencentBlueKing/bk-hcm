...
```go
func SQL(format string, ele ...any) string {
    sql := fmt.Sprintf(format, ele...)
    // 移除多余空白字符
    // ...
}

// From: pkg/dal/dao/helper/sql.go:69-72
func SQLSum(fieldName string) string {
    return fmt.Sprintf("IFNULL(ROUND(SUM(%s), %d), 0)", fieldName, constant.FloatDefaultPointPrecision)
}
```
...
expr.Rules = append(expr.Rules, filter.EqualRule("type", typ))
expr.Rules = append(expr.Rules, filter.EqualRule("name", name))
```
