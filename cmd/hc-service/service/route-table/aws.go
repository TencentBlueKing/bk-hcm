...
// AwsRouteTableUpdate update aws route table.
func (r routeTable) AwsRouteTableUpdate(cts *rest.Contexts) (interface{}, error) {
	...
	getRes, err := r.cs.DataService().Aws.RouteTable.Get(cts.Kit.Ctx, cts.Kit.Header(), id)
	if err != nil {
		return nil, err
	}

	cli, err := r.ad.Aws(cts.Kit, getRes.AccountID)
	if err != nil {
		return nil, err
	}

	updateOpt := new(routetable.AwsRouteTableUpdateOption)
...
// AwsRouteTableDelete delete aws route table.
func (r routeTable) AwsRouteTableDelete(cts *rest.Contexts) (interface{}, error) {
	id := cts.PathParameter("id").String()
