...
func (hd *imageHandler) Next(kt *kit.Kit) ([]string, error) {
	listOpt := &image.AwsImageListOption{
	...
	if err != nil {
		logs.Errorf("request adaptor list aws image failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
	...
	hd.nextToken = imageResult.NextToken
	return cloudIDs, nil
}
...
func (hd *imageHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.Image(kt, params, new(aws.SyncImageOption)); err != nil {
		logs.Errorf("sync aws image failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
