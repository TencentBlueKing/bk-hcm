...
func (hd *vpcHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list huawei vpc failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
	...
	hd.marker = converter.ValToPtr(vpcResult.Details[len(vpcResult.Details)-1].CloudID)
	return cloudIDs, nil
}
...
func (hd *vpcHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.Vpc(kt, params, new(huawei.SyncVpcOption)); err != nil {
		logs.Errorf("sync huawei vpc failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
