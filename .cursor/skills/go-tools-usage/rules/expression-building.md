...
| `AndExpression()`/`OrExpression()` | 合并多个已有表达式或规则 | ⭐⭐⭐ 推荐 |
...
## 4. 推荐方式：AndExpression()/OrExpression() 合并规则
...
```go
expr, err := filter.AndExpression(
    filt,
    filter.VarTimeRangeExpression("time", vtr),
)
if err != nil {
    return nil, err
}
...
```go
orExpr, err := filter.OrExpression(
    filter.EqualRule("type", "A"),
    filter.EqualRule("type", "B"),
    filter.EqualRule("type", "C"),
)
```
...
f.Rules = append(f.Rules, &filter.Expression{
    ...
    Rules: []filter.RuleFactory{
        filter.NotEqualExpression("platform_category_id", enumor.PlatCatBGService),
        filter.NotContainExpression("platform_id", opt.InternalAllocPlatIDs),
    },
})
```
...
expr := &filter.Expression{
    Op: filter.And,
    Rules: []filter.RuleFactory{
        filter.MonthExpression("stat_date", month),
        filter.GreaterThanExpression("os", 0),
    },
}
```
...
expr.Rules = append(expr.Rules, filter.EqualRule("name", "test"))
```
...
| 函数 | 用途 | 返回类型 |
|------|------|----------|
| `RuleEqual(field, value)` | 等于规则 | `RuleFactory` |
| `RuleNotEqual(field, value)` | 不等于规则 | `RuleFactory` |
| `RuleGreaterThanEqual(field, value)` | 大于等于规则 | `RuleFactory` |
| `RuleLessThanEqual(field, value)` | 小于等于规则 | `RuleFactory` |
| `RuleIn(field, values)` | IN 规则 | `RuleFactory` |
| `RuleNotIn(field, values)` | NOT IN 规则 | `RuleFactory` |
| `RuleJsonOverlaps(field, values)` | JSON 数组交集规则 | `RuleFactory` |
| `RuleJSONContains[T any](fieldName string, values T)` | JSON 数组包含规则 | `RuleFactory` |
