...
// TCloudRouteTableUpdate update tencent cloud route table.
func (r routeTable) TCloudRouteTableUpdate(cts *rest.Contexts) (interface{}, error) {
	...
	getRes, err := r.cs.DataService().TCloud.RouteTable.Get(cts.Kit.Ctx, cts.Kit.Header(), id)
	if err != nil {
		return nil, err
	}

	cli, err := r.ad.TCloud(cts.Kit, getRes.AccountID)
	if err != nil {
		return nil, err
	}

	updateOpt := new(routetable.TCloudRouteTableUpdateOption)
...
// TCloudRouteTableDelete delete tencent cloud route table.
func (r routeTable) TCloudRouteTableDelete(cts *rest.Contexts) (interface{}, error) {
	id := cts.PathParameter("id").String()
