...
func (c *Command) Help() *Help {
	...
	if len(c.Parameters) == 0 {
		h.Example = `curl -XPOST http://127.0.0.1/tools?cmd=[command]`
		return h
	}
	...
	if c.FromURL {
		h.Example = `curl -XPOST http://127.0.0.1/tools?cmd=[command][&parameter=value]'`
	} else {
		h.Example = `curl -XPOST http://127.0.0.1/tools?cmd=[command] -d '{[parameter:value]}'`
	}
