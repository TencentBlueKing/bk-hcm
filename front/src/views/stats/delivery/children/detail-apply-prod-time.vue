...
const fetchDetailData = async () => {
  loading.value = true;
  const res = await http.post('/api/v1/woa/task/apply/analysis/production_stage_time_cost/compare', {
  ...
  detailData.value = mergeTableData(currentData, compareData, ['avg_duration_hours', 'done_orders']);
...
const columns: Ref<TableProps['columns']> = computed(() => [
  ...
  {
    title: '生产阶段耗时',
    colKey: 'avg_duration_hours',
    align: 'center',
    children: [
      {
        title: currentMonth.value,
        colKey: 'current_avg_duration_hours',
        sortType: 'all',
        sorter: (a, b) => a.current_avg_duration_hours - b.current_avg_duration_hours,
        cell: (h, { row }) =>
          row?.current_avg_duration_hours !== undefined ? `${row?.current_avg_duration_hours}小时` : '--',
      },
      {
        title: compareMonth.value,
        colKey: 'compare_avg_duration_hours',
        sortType: 'all',
        sorter: (a, b) => a.compare_avg_duration_hours - b.compare_avg_duration_hours,
        cell: (h, { row }) =>
          row?.compare_avg_duration_hours !== undefined ? `${row?.compare_avg_duration_hours}小时` : '--',
      },
      {
        title: '环比',
        colKey: 'growth_rate',
        ...
        sortType: 'all',
        sorter: (a, b) => {
          const aValue = calculateGrowthRate(a.current_avg_duration_hours, a.compare_avg_duration_hours).value;
          const bValue = calculateGrowthRate(b.current_avg_duration_hours, b.compare_avg_duration_hours).value;
          return aValue - bValue;
        },
        cell: (h, { row }) => {
          const rate = calculateGrowthRate(row?.current_avg_duration_hours, row?.compare_avg_duration_hours);
          return h(
            'div',
            {
              class: rate?.class,
