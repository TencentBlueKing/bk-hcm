...
hc-service SHALL 提供 GPU 实例类型查询接口，接收 `cloud_id`（成员账号 AWS Account ID）、`role_chain`（角色名数组，支持 Role Chaining）、`region` 参数。系统通过 CloudID 反查 sub_account 获取根账号 AK/SK，再按 role_chain 顺序执行链式 AssumeRole 获取成员账号权限后，调用 `DescribeInstanceTypes` 并返回含 GPU 字段的实例类型列表。该接口为 AWS 数据透传，不持久化到本地数据库。仅涉及 AWS 云厂商。

#### Scenario: 查询 GPU 实例类型成功
- **GIVEN** cloud_id、role_chain、region 均有效，且 cloud_id 在 sub_account 表中存在
...
hc-service SHALL 提供 GPU 实例列表查询接口，接收 `cloud_id`（成员账号 AWS Account ID）、`role_chain`（角色名数组，支持 Role Chaining）、`region` 参数。系统通过 CloudID 反查 sub_account 获取根账号 AK/SK，再按 role_chain 顺序执行链式 AssumeRole 获取成员账号权限后，调用 `DescribeInstances` 并返回该 region 下的 EC2 实例列表。该接口为 AWS 数据透传，不持久化到本地数据库。仅涉及 AWS 云厂商。
...
cloud-server SHALL 提供 GPU 实例类型查询和 GPU 实例列表查询的资源视角入口 handler。入口 handler SHALL 执行 `ResOperateAuth` 鉴权后，通过微服务调用 hc-service 对应接口。路由注册于 cloud-server 的资源视角路由组。仅涉及 AWS 云厂商。
...
1. 以 `cloud_id`（成员账号 AWS Account ID）+ `role_chain`（角色名数组）+ `region` 直接调用 GPU 数据接口
2. HCM 内部自动用 CloudID 反查 sub_account 表找到根账号，按 role_chain 顺序完成链式 AssumeRole

下游平台无需感知 HCM 内部的 account_id 和 sub_account_id。仅涉及 AWS 云厂商。

#### Scenario: 单步调用
- **GIVEN** 下游平台持有 AWS Account ID "123456789012"，role_chain 为 `["gpu-readonly"]`
- **WHEN** 以 cloud_id + role_chain + region 调用 GPU 实例类型接口和 GPU 实例列表接口
- **THEN** 成功获取目标成员账号在该 region 的实例类型规格和实例列表

#### Scenario: 多步 Role Chain 调用
- **GIVEN** 下游平台持有 AWS Account ID "123456789012"，role_chain 为 `["GPUInventoryCallerRole", "GPUInventoryReadOnlyRole"]`
- **WHEN** 以 cloud_id + role_chain + region 调用 GPU 数据接口
- **THEN** HCM 先在管理账号中 Assume GPUInventoryCallerRole，再在成员账号中 Assume GPUInventoryReadOnlyRole，成功获取数据

#### Scenario: CloudID 未同步
- **GIVEN** 下游平台传入的 AWS Account ID 在 HCM 的 sub_account 表中不存在
- **WHEN** 调用 GPU 数据接口
- **THEN** 返回错误提示该 CloudID 未同步，下游平台可告知运维检查 sub_account 同步状态
