...
func (ad Audit) permissionPolicyLibraryUpdateAuditBuild(kt *kit.Kit, updates []protoaudit.CloudResourceUpdateInfo) (
	...
	idMap, err := ad.listPermissionPolicyLibrary(kt, ids)
	if err != nil {
		return nil, err
	}
	...
	for _, one := range updates {
		...
		audits = append(audits, &tableaudit.AuditTable{
			...
			Vendor:   enumor.Vendor(resData.Vendor),
			Operator: kt.User,
...
func (ad Audit) permissionPolicyLibraryDeleteAuditBuild(kt *kit.Kit, deletes []protoaudit.CloudResourceDeleteInfo) (
	[]*tableaudit.AuditTable, error) {
...
func (ad Audit) listPermissionPolicyLibrary(kt *kit.Kit,
	...
	list, err := ad.dao.PermissionPolicyLibrary().List(kt, opt)
	if err != nil {
		logs.Errorf("list permission_policy_library failed, err: %v, ids: %v, rid: %s", err, ids, kt.Rid)
		return nil, err
	}

	result := make(map[string]*tablecloud.PermissionPolicyLibraryTable, len(list.Details))
