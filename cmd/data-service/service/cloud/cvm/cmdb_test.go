...
// require an account lookup. UnassignedBiz hosts are not filtered here (the upsert layer skips
// pushing them), so a resource pool host still gets its creator as operator.
func TestBuildCmdbOperatorsWithoutAccountLookup(t *testing.T) {
	...
	models := []*cvm.Table{
		...
		// resource pool host with creator -> operator still derived (unassigned biz not filtered here)
	...
	if len(operators) != 2 {
		t.Fatalf("expect 2 operators, got: %d, operators: %+v", len(operators), operators)
	}
	if operators["cvm-1"] == nil || *operators["cvm-1"] != "zhangsan" {
		t.Errorf("expect cvm-1 operator zhangsan, got: %+v", operators["cvm-1"])
	}
	if operators["cvm-4"] == nil || *operators["cvm-4"] != "wangwu" {
		t.Errorf("expect cvm-4 operator wangwu, got: %+v", operators["cvm-4"])
	}
	for _, cvmID := range []string{"cvm-2", "cvm-3", "cvm-5"} {
