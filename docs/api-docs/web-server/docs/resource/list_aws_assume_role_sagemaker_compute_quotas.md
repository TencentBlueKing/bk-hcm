...
- 该接口提供版本：9.9.9。
- 该接口所需权限：资源查看。
- 该接口功能描述：查询 AWS 成员账号 SageMaker HyperPod Compute Quota 列表。通过 STS AssumeRole 跨账号访问成员账号的 SageMaker 原生接口，透传返回 AWS 原始响应。

### URL

POST /api/v1/cloud/vendors/aws/assume_role/sagemaker/compute_quotas/list
...
| cluster_arn | string | 否 | HyperPod cluster ARN 过滤 |
| created_after | string | 否 | 创建时间下界，RFC3339 时间格式 |
| created_before | string | 否 | 创建时间上界，RFC3339 时间格式 |
...
```json
{
  ...
  "region": "us-east-1",
  "cluster_arn": "arn:aws:sagemaker:us-east-1:123456789012:cluster/demo"
}
```
...
```json
{
  "code": 0,
  "message": "",
  "data": {"ComputeQuotaSummaries":[{"ComputeQuotaId":"cq-123","Name":"demo-quota"}]}
}
```
...
| code | int | 状态码 |
| message | string | 请求信息 |
| data | object | AWS SageMaker `ListComputeQuotas` 原始响应透传 |
