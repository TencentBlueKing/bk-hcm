...
| 参数名称      | 参数类型      | 必选 | 描述                                            |
|-------------|--------------|------|------------------------------------------------|
| vendor      | string       | 是   | 云厂商（枚举值：`tcloud`、`huawei`）               |
...
6. 智能分组查询：系统会根据实例ID自动识别所属的账号（account_id）和地域（region），并按照 `(account_id, region)` 组合进行分组，对每个分组单独调用云厂商API查询监控数据
7. 跨账号跨地域支持：支持查询不同账号、不同地域的实例监控数据，系统会自动处理分组和聚合
8. 返回数据说明：
   - `id`：HCM系统内部的实例ID
