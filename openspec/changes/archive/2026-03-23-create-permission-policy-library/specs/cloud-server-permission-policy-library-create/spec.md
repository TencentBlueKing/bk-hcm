...
请求字段：`name`（string，必填）、`policy_document`（string，必填）、`bk_biz_ids`（int64 array，必填）、`memo`（string，可选）。
...
- `PermissionPolicyLibraryCreateReq`：`name`（required, max=128）、`policy_document`（required）、`bk_biz_ids`（required）、`memo`（omitempty, max=255）
- `PermissionPolicyLibraryCreateResult`：`id`（string）

#### Scenario: 请求模型校验

- **WHEN** 请求解码成功后调用 Validate()
- **THEN** 系统 SHALL 校验 name 非空且不超过 128 字符、policy_document 非空、bk_biz_ids 非 nil
