# Spec: prediction-transit-pool-split-before-transfer

## Purpose

在转移执行前，通过贪心降序组合算法精确选出待转移的 CRP 单，当候选核数与需求不精确匹配时以中转池产品身份调用 `adjustOrder` 拆分大预测，保证最终转移量与业务需求精确一致。同时限制转移逻辑仅对技术运营部（VirtualDeptID == 1041）的 ticket 生效。
