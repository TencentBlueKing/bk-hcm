...
func (cli *client) deleteRoute(kt *kit.Kit, accountID, region, routeTableID string,
	...
	if len(delRouteFromCloud) > 0 {
		logs.Errorf("[%s] validate route not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloud, checkParams, len(delRouteFromCloud), kt.Rid)
	...
	if err := cli.dbCli.TCloud.RouteTable.BatchDeleteRoute(kt.Ctx, kt.Header(), routeTableID, deleteReq); err != nil {
...
func (cli *client) listRouteFromDB(kt *kit.Kit, opt *syncRouteOption) ([]routetable.TCloudRoute, error) {
	if err := opt.Validate(); err != nil {
		return nil, errf.NewFromErr(errf.InvalidParameter, err)
	}
	...
	results, err := cli.dbCli.TCloud.RouteTable.ListRoute(kt.Ctx, kt.Header(), opt.RouteTableID, req)
	if err != nil {
		logs.Errorf("[%s] batch list route failed. err: %v, accountID: %s, region: %s, "+
			"routeTableID: %s, rid: %s", enumor.TCloud, err, opt.AccountID, opt.Region, opt.RouteTableID, kt.Rid)
		return nil, err
	}
