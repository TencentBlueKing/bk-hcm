...
func SyncAllResource(kt *kit.Kit, cliSet *client.ClientSet,
	...
	start := time.Now()
	logs.V(3).Infof("huawei account[%s] sync all resource start, time: %v, opt: %v, rid: %s", opt.AccountID,
		start, opt, kt.Rid)
	...
	if hitErr = SyncDisk(kt, cliSet, opt.AccountID); hitErr != nil {
	...
	if hitErr = SyncSubnet(kt, cliSet, opt.AccountID); hitErr != nil {
		return enumor.SubnetCloudResType, hitErr
	}

	if hitErr = SyncEip(kt, cliSet, opt.AccountID); hitErr != nil {
		return enumor.EipCloudResType, hitErr
	}

	if hitErr = SyncSG(kt, cliSet, opt.AccountID); hitErr != nil {
		return enumor.SecurityGroupCloudResType, hitErr
	}

	if hitErr = SyncCvm(kt, cliSet, opt.AccountID); hitErr != nil {
		return enumor.CvmCloudResType, hitErr
	}

	if hitErr = SyncRouteTable(kt, cliSet, opt.AccountID); hitErr != nil {
