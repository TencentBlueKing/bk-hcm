...
// AccountBillSyncRecord only used for interface.
type AccountBillSyncRecord interface {
	CreateWithTx(kt *kit.Kit, tx *sqlx.Tx, regions []tablebill.AccountBillSyncRecord) ([]string, error)
	List(kt *kit.Kit, opt *types.ListOption) (*typesbill.ListAccountBillSyncRecordDetails, error)
	UpdateByIDWithTx(kt *kit.Kit, tx *sqlx.Tx, billID string, updateData *tablebill.AccountBillSyncRecord) error
	DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, filterExpr *filter.Expression) error
}

// AccountBillSyncRecordDao account bill sync record dao
type AccountBillSyncRecordDao struct {
	Orm   orm.Interface
	IDGen idgenerator.IDGenInterface
}

// CreateWithTx create account bill sync record with tx.
func (a AccountBillSyncRecordDao) CreateWithTx(
	kt *kit.Kit, tx *sqlx.Tx, models []tablebill.AccountBillSyncRecord) (
	[]string, error) {
...
func (a AccountBillSyncRecordDao) List(kt *kit.Kit, opt *types.ListOption) (
	*typesbill.ListAccountBillSyncRecordDetails, error) {

	if opt == nil {
		return nil, errf.New(errf.InvalidParameter, "list account bill sync record options is nil")
	}

	if err := opt.Validate(filter.NewExprOption(
		filter.RuleFields(tablebill.AccountBillSyncRecordColumns.ColumnTypes())),
	...
	if opt.Page.Count {
		...
		return &typesbill.ListAccountBillSyncRecordDetails{Count: count}, nil
	}
	...
	sql := fmt.Sprintf(`SELECT %s FROM %s %s %s`,
		tablebill.AccountBillSyncRecordColumns.FieldsNamedExpr(opt.Fields),
	...
	return &typesbill.ListAccountBillSyncRecordDetails{Details: details}, nil
}

// UpdateByIDWithTx update account bill sync record.
func (a AccountBillSyncRecordDao) UpdateByIDWithTx(kt *kit.Kit, tx *sqlx.Tx, id string,
	updateData *tablebill.AccountBillSyncRecord) error {
...
// DeleteWithTx delete account bill sync record with tx.
func (a AccountBillSyncRecordDao) DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression) error {
	if expr == nil {
