...
func (s *SecurityGroup) azureSGRuleUpdateAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
	...
	idSgRuleMap, err := s.listAzureSGRule(kt, sg.ID, ids)
	if err != nil {
		return nil, err
	}
...
// AzureSGRuleDeleteAuditBuild azure rule delete audit build.
func (s *SecurityGroup) azureSGRuleDeleteAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
...
func (s *SecurityGroup) listAzureSGRule(kt *kit.Kit, sgID string, ids []string) (
	map[string]tablecloud.AzureSecurityGroupRuleTable, error) {
	...
	if err != nil {
		logs.Errorf("list azure security group rule failed, err: %v, ids: %v, rid: %s", err, ids, kt.Rid)
		return nil, err
	}

	result := make(map[string]tablecloud.AzureSecurityGroupRuleTable, len(list.Details))
