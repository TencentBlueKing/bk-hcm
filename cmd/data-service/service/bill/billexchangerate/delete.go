...
func (svc *service) BatchDeleteExchangeRate(cts *rest.Contexts) (interface{}, error) {
	...
	listResp, err := svc.dao.AccountBillExchangeRate().List(cts.Kit, opt)
	if err != nil {
		logs.Errorf("delete list account bill exchange rate failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return nil, fmt.Errorf("delete list account bill exchange rate failed, err: %v", err)
	}
