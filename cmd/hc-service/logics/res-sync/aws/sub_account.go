...
func (cli *client) SubAccount(kt *kit.Kit, opt *SyncSubAccountOption) (*SyncResult, error) {
	...
	addSlice, updateMap, delCloudIDs := common.Diff[account.AwsAccount,
		coresubaccount.SubAccount[coresubaccount.AwsExtension]](fromCloud, fromDB, isSubAccountChange)
...
func (cli *client) updateSubAccount(kt *kit.Kit, opt *SyncSubAccountOption,
	updateMap map[string]account.AwsAccount) error {

	if len(updateMap) <= 0 {
		return errors.New("updateMap is required")
	}

	account, err := cli.dbCli.Aws.Account.Get(kt.Ctx, kt.Header(), opt.AccountID)
	if err != nil {
	...
	for id, one := range updateMap {
		extension := &coresubaccount.AwsExtension{
		...
		accountType := ""
		if account.Extension.CloudAccountID != "" &&
			account.Extension.CloudAccountID == converter.PtrToVal(one.ID) {
	...
	if err = cli.dbCli.Global.SubAccount.BatchUpdate(kt, updateReq); err != nil {
		logs.Errorf("[%s] update sub account failed, err: %v, account: %s, opt: %v, rid: %s", enumor.Aws,
			err, opt.AccountID, opt, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync sub account to update sub account success, accountID: %s, count: %d, rid: %s", enumor.Aws,
		opt.AccountID, len(updateMap), kt.Rid)

	return nil
}

func (cli *client) createSubAccount(kt *kit.Kit, opt *SyncSubAccountOption, addSlice []account.AwsAccount) error {
	...
	if _, err = cli.dbCli.Global.SubAccount.BatchCreate(kt, createReq); err != nil {
		logs.Errorf("[%s] create sub account failed, err: %v, account: %s, opt: %v, rid: %s", enumor.Aws,
			err, opt.AccountID, opt, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync sub account to create sub account success, accountID: %s, count: %d, rid: %s", enumor.Aws,
...
func (cli *client) deleteSubAccount(kt *kit.Kit, opt *SyncSubAccountOption, delCloudIDs []string) error {
	...
	for _, one := range delFromCloud {
		if _, exsit := delCloudMap[one.GetCloudID()]; exsit {
			logs.Errorf("[%s] validate account not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
				enumor.Aws, opt, len(delFromCloud), kt.Rid)
	...
	for _, parts := range elems {
		...
		if err != nil {
			logs.Errorf("[%s] delete sub account failed, err: %v, account: %s, opt: %v, rid: %s", enumor.Aws,
				err, opt.AccountID, opt, kt.Rid)
			return err
		}
	}

	logs.Infof("[%s] sync sub account to delete sub account success, accountID: %s, count: %d, rid: %s", enumor.Aws,
		opt.AccountID, len(delCloudIDs), kt.Rid)

	return nil
}

func isSubAccountChange(cloud account.AwsAccount, db coresubaccount.SubAccount[coresubaccount.AwsExtension]) bool {

	if !assert.IsPtrStringEqual(cloud.Arn, db.Extension.Arn) {
		return true
	}
	...
	if !assert.IsPtrStringEqual(cloud.Status, db.Extension.Status) {
		return true
	}
...
func (cli *client) listSubAccountFromCloud(kt *kit.Kit, opt *SyncSubAccountOption) ([]account.AwsAccount, error) {
	if err := opt.Validate(); err != nil {
		return nil, errf.NewFromErr(errf.InvalidParameter, err)
	}
	...
	if err != nil {
		logs.Errorf("[%s] list sub account from cloud failed, err: %v, account: %s, opt: %v, rid: %s", enumor.Aws,
			err, opt.AccountID, opt, kt.Rid)
		return nil, err
	}
...
func (cli *client) listSubAccountFromDB(kt *kit.Kit, opt *SyncSubAccountOption) (
	[]coresubaccount.SubAccount[coresubaccount.AwsExtension], error) {
	...
	start := uint32(0)
	results := make([]coresubaccount.SubAccount[coresubaccount.AwsExtension], 0)
	for {
		req.Page.Start = start
		resp, err := cli.dbCli.Aws.SubAccount.ListExt(kt.Ctx, kt.Header(), req)
		if err != nil {
			logs.Errorf("[%s] list sub account from db failed, err: %v, account: %s, req: %v, rid: %s",
				enumor.Aws, err, opt.AccountID, req, kt.Rid)
			return nil, err
		}
