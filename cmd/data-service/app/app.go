...
func (ds *dataService) prepare(ctx context.Context, opt *options.Option) error {
	...
	// init trace
	if cc.DataService().Trace.Enabled {
		if err := traces.InitTracer(ctx, cc.DataService().Trace.ToTraceOption()); err != nil {
