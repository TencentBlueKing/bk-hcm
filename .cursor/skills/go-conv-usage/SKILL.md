...
### Q: `GroupBy` 和 `SliceToMap` 的区别？
A: `GroupBy` 一个 key 对应多个值（切片），用于分组；`SliceToMap` 一个 key 对应一个值，用于查找。

### Q: 数值转换函数支持哪些格式的字符串？
A: 支持标准数值字符串（如 "123"）和千位分隔符格式（如 "1,234,567.89"），不支持科学计数法。

### Q: `SliceToMap` 的 `overwrite` 参数如何选择？
A: `overwrite=false` 遇到重复 key 返回错误，用于严格映射；`overwrite=true` 允许覆盖，用于更新场景。
