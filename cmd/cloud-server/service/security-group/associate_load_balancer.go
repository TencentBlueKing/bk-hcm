...
func (svc *securityGroupSvc) batchAssociateLoadBalancers(cts *rest.Contexts,
	...
	case enumor.TCloud:
		...
		return nil, nil
	case enumor.TCloudZiyan:
		err = svc.client.HCService().TCloudZiyan.SecurityGroup.BatchAssociateLoadBalancers(cts.Kit, req)
		if err != nil {
			logs.Errorf("fail to call tcloud-ziyan hc service associate cloud load balancers, err: %v, sgID: %s, "+
...
func (svc *securityGroupSvc) batchDisassociateLoadBalancers(cts *rest.Contexts,
	...
	case enumor.TCloud:
		...
		return nil, nil
	case enumor.TCloudZiyan:
		err = svc.client.HCService().TCloudZiyan.SecurityGroup.BatchDisassociateLoadBalancers(cts.Kit, req)
		if err != nil {
			logs.Errorf("fail to call tcloud-ziyan hc service disassociate cloud load balancers, err: %v, sgID: %s, "+
				"req: %+v, rid: %s", err, req.SecurityGroupID, cvt.PtrToVal(req), cts.Kit.Rid)
			return nil, err
		}
		return nil, nil
