...
func (dao *AsyncFlowDao) Create(kt *kit.Kit, tx *sqlx.Tx, model *tableasync.AsyncFlowTable) (string, error) {
	...
	if err = dao.Orm.ModifySQLOpts(orm.NewInjectTenantIDOpt(kt.TenantID)).Txn(tx).
		BulkInsert(kt.Ctx, sql, model); err != nil {
		logs.Errorf("insert %s failed, err: %v, sql: %s, rid: %s",
			table.AsyncFlowTable, err, sql, kt.Rid)
...
func (dao *AsyncFlowDao) ListWithTx(kt *kit.Kit, tx *sqlx.Tx,
	...
	if opt.Page.Count {
		...
		count, err := dao.Orm.ModifySQLOpts(orm.NewInjectTenantIDOpt(kt.TenantID)).Txn(tx).Count(kt.Ctx, sql,
			whereValue)
		if err != nil {
...
func (dao *AsyncFlowDao) DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, filterExpr *filter.Expression) error {
	...
	if _, err = dao.Orm.ModifySQLOpts(orm.NewInjectTenantIDOpt(kt.TenantID)).Txn(tx).Delete(kt.Ctx, sql, whereValue); err != nil {
