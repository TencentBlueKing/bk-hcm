...
// SyncAzureNetworkInterface sync azure network interface to hcm.
func (v syncNetworkInterfaceSvc) SyncAzureNetworkInterface(cts *rest.Contexts) (interface{}, error) {
	...
	if err != nil {
		logs.Errorf("request to sync azure network interface logic failed, req: %+v, err: %v, rid: %s",
			req, err, cts.Kit.Rid)
		return nil, err
	}
