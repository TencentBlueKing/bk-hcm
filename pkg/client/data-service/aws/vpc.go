...
func (v *VpcClient) BatchCreate(ctx context.Context, h http.Header,
	req *protocloud.VpcBatchCreateReq[corecloud.AwsVpcExtension]) (*core.BatchCreateResult, error) {
	...
	err := v.client.Post().
		WithContext(ctx).
		Body(req).
		SubResourcef("/vendors/%s/vpcs/batch/create", enumor.Aws).
		WithHeaders(h).
		Do().
		Into(resp)
	if err != nil {
...
func (v *VpcClient) BatchUpdate(ctx context.Context, h http.Header,
	...
	err := v.client.Patch().
		WithContext(ctx).
		Body(req).
		SubResourcef("/vendors/%s/vpcs/batch", enumor.Aws).
		WithHeaders(h).
		Do().
		Into(resp)
	if err != nil {
