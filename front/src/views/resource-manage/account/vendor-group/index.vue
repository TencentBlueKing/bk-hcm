...
const accountId = ref<string>((route.query.accountId as string) || '');
const currentVendor = ref<VendorEnum | null>(null);
...
watch(
  () => route.query.vendor,
  (vendor) => {
    currentVendor.value = (vendor as VendorEnum) || null;
  },
  {
    immediate: true,
  },
  ...
  <div class="account-vendor-group">
    ...
    <div class="account-container" v-bkloading="{ loading: authorizedResourceAccountLoading }">
      <template v-if="!isAccountAllEmpty">
        <div
          :class="['account-all', { active: !accountId && !currentVendor }]"
