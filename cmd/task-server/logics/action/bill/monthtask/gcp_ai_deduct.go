...
// GcpAIDeductMonthTask ...
type GcpAIDeductMonthTask struct {
	gcpMonthTaskBaseRunner
}

// GetBatchSize ..
func (a *GcpAIDeductMonthTask) GetBatchSize(kt *kit.Kit) uint64 {
...
// Split ...
func (a *GcpAIDeductMonthTask) Split(kt *kit.Kit, opt *MonthTaskActionOption,
	rawItemList []*bill.RawBillItem) ([]bill.BillItemCreateReq[json.RawMessage], error) {

	if len(rawItemList) == 0 {
		return nil, nil
	}
	err := a.initExtension(opt)
	if err != nil {
		logs.Errorf("fail to init extension, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
	...
	for i := range rawItemList {
		item := rawItemList[i]
		accountCloudId := gjson.Get(string(item.Extension), "project_id").String()
		if len(accountCloudId) == 0 {
			return nil, fmt.Errorf("empty project_id for idx: %d", i)
		}
		summaryMain := cloudIdSummaryMainMap[accountCloudId]
		if summaryMain == nil {
			logs.Errorf("can not found main account(%s) for gcp ai deduct split, rid: %s",
				accountCloudId, kt.Rid)
			return nil, fmt.Errorf("can not found main account(%s) for gcp ai deduct split", accountCloudId)
		}
		usageBillItem := bill.BillItemCreateReq[json.RawMessage]{
			...
			HcProductCode: constant.GcpAIDeductProductCode,
			HcProductName: constant.GcpAIDeductProductCode,
			ResAmount:     item.ResAmount,
			ResAmountUnit: item.ResAmountUnit,
...
// GetHcProductCodes ...
func (a *GcpAIDeductMonthTask) GetHcProductCodes() []string {
	return []string{constant.GcpAIDeductProductCode}
}
