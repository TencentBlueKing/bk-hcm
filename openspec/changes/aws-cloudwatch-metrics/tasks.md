...
- [x] 2.1 `pkg/api/hc-service/instance-type/aws.go`（或新增文件）：定义 `AwsAssumeRoleGetMetricDataReq`（cloud_id + role_chain + region + metric_data_queries + start_time + end_time）
...
- [x] 2.4 定义 `AwsAssumeRoleListMetricsReq`（cloud_id + role_chain + region + namespace + metric_name + dimensions）
- [x] 2.5 定义 `AwsAssumeRoleListMetricsResp`（Metrics 数组，每个包含 Namespace、MetricName、Dimensions）

## 3. hc-service CloudWatch handler

- [x] 3.1 `cmd/hc-service/service/`：实现 GetMetricData handler（AwsWithAssumeRole → adaptor.GetMetricData）
- [x] 3.2 实现 ListMetrics handler（AwsWithAssumeRole → adaptor.ListMetrics）
...
- [x] 5.1 `cmd/cloud-server/service/`：新增 GetMetricData 资源视角 handler（含 CloudID→AccountID 反查 + ResOperateAuth 鉴权）
- [x] 5.2 新增 ListMetrics 资源视角 handler
...
- [x] 6.1 `docs/api-docs/web-server/docs/resource/get_aws_assume_role_metric_data.md`：GetMetricData 接口文档
- [x] 6.2 `docs/api-docs/web-server/docs/resource/list_aws_assume_role_metrics.md`：ListMetrics 接口文档
