# create-gpu-demand-order

## Purpose

Defines the API and data flow for creating GPU demand orders (main order + sub-orders) from frontend-provided details.

## Requirements

### Requirement: Create GPU demand order API endpoint
The system SHALL provide a POST endpoint at `/api/v1/woa/bizs/{bk_biz_id}/plans/resources/gpu/order/create` that creates a GPU demand order (main order) and its associated sub-orders from the details provided by the frontend.
