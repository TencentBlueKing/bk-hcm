...
func (svc *clbSvc) BatchRemoveTCloudListenerTargets(cts *rest.Contexts) (any, error) {
	...
	targetIDs, cloudLblIDs, err := svc.unbindCloudListenerTargets(cts.Kit, req, lblRsList)
	if err != nil {
		return nil, err
	}
...
func (svc *clbSvc) unbindCloudListenerTargets(kt *kit.Kit, req *protolb.TCloudBatchUnbindRsReq,
	lblRsList []*dataproto.ListBatchListenerResult) ([]string, []string, error) {

	tcloudAdpt, err := svc.ad.TCloud(kt, req.AccountID)
	if err != nil {
		logs.Errorf("listener unbind rs api failed, get account failed, err: %v, accountID: %s, rid: %s",
	...
	if len(failIDs) > 0 {
		logs.Errorf("listener unbind rs api partially failed, failLblIDs: %v, req: %+v, "+
