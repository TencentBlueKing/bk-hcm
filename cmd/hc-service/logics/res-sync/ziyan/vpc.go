...
func (cli *client) Vpc(kt *kit.Kit, params *SyncBaseParams, opt *SyncVpcOption) (*SyncResult, error) {
	...
	addVpc, updateMap, delCloudIDs := common.Diff[types.TCloudVpc, cloudcore.Vpc[cloudcore.TCloudVpcExtension]](
		vpcFromCloud, vpcFromDB, isTCloudZiyanVpcChange)
...
func (cli *client) RemoveVpcDeleteFromCloud(kt *kit.Kit, accountID string, region string) error {
	...
	for {
		...
		if err != nil {
			logs.Errorf("[%s] request dataservice to list vpc failed, err: %v, req: %v, rid: %s",
...
func (cli *client) deleteVpc(kt *kit.Kit, accountID string, region string, delCloudIDs []string) error {
	...
	if len(delVpcFromCloud) > 0 {
		logs.Errorf("[%s] validate vpc not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloudZiyan, checkParams, len(delVpcFromCloud), kt.Rid)
	...
	if err = cli.dbCli.Global.Vpc.BatchDelete(kt.Ctx, kt.Header(), deleteReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch delete vpc failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync vpc to delete vpc success, accountID: %s, count: %d, rid: %s",
...
func (cli *client) updateVpc(kt *kit.Kit, accountID string, updateMap map[string]types.TCloudVpc) error {
	...
	if err := cli.dbCli.TCloudZiyan.Vpc.BatchUpdate(kt.Ctx, kt.Header(), updateReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch update db vpc failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync vpc to update vpc success, accountID: %s, count: %d, rid: %s",
		enumor.TCloudZiyan, accountID, len(updateMap), kt.Rid)
...
func (cli *client) createVpc(kt *kit.Kit, accountID string, addVpc []types.TCloudVpc) error {
	...
	if _, err := cli.dbCli.TCloudZiyan.Vpc.BatchCreate(kt.Ctx, kt.Header(), createReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch create vpc failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync vpc to create vpc success, accountID: %s, count: %d, rid: %s",
		enumor.TCloudZiyan, accountID, len(addVpc), kt.Rid)
...
func (cli *client) listVpcFromCloud(kt *kit.Kit, params *SyncBaseParams) ([]types.TCloudVpc, error) {
	...
	if err != nil {
		logs.Errorf("[%s] list vpc from cloud failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloudZiyan, err,
...
func (cli *client) listVpcFromDB(kt *kit.Kit, params *SyncBaseParams) (
	...
	result, err := cli.dbCli.TCloudZiyan.Vpc.ListVpcExt(kt.Ctx, kt.Header(), req)
	if err != nil {
		logs.Errorf("[%s] list vpc from db failed, err: %v, account: %s, req: %v, rid: %s",
...
func isTCloudZiyanVpcChange(item types.TCloudVpc, info cloudcore.Vpc[cloudcore.TCloudVpcExtension]) bool {
