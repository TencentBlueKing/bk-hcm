...
func (hd *eipHandler) Next(kt *kit.Kit) ([]string, error) {
	listOpt := &typeseip.GcpEipListOption{
	...
	if err != nil {
		logs.Errorf("request adaptor list gcp eip failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
	...
	hd.pageToken = eipResult.NextPageToken
	return cloudIDs, nil
}
...
func (hd *eipHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	opt := &gcp.SyncEipOption{
		Region: hd.request.Region,
	}
	if _, err := hd.syncCli.Eip(kt, params, opt); err != nil {
		logs.Errorf("sync gcp eip failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
...
func (hd *eipHandler) RemoveDeleteFromCloud(kt *kit.Kit) error {
	if err := hd.syncCli.RemoveEipDeleteFromCloud(kt, hd.request.AccountID, hd.request.Region); err != nil {
		logs.Errorf("remove eip delete from cloud failed, err: %v, accountID: %s, rid: %s", err,
