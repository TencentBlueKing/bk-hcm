# Spec: aws-assume-role

## Purpose

通过 AWS STS AssumeRole 实现跨账号访问能力，支持 Role Chain 场景。包括临时凭证获取、Role ARN 自动拼接、进程内缓存、AwsWithAssumeRole 编排方法，以及 BaseSecret 对 SessionToken 的支持。
