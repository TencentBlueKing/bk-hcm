		...
		deleteReq := &dataservice.BatchDeleteReq{
			Filter: &filter.Expression{
				...
				Rules: []filter.RuleFactory{
					&filter.AtomRule{
						Field: "extension.uid",
						Op:    filter.JSONOverlaps.Factory(),
						Value: converter.StringSliceToUint64Slice(parts),
					},
				},
			},
		}
...
func (cli *client) listAccountFromCloud(kt *kit.Kit, opt *SyncSubAccountOption) ([]account.AwsAccount, error) {
	if err := opt.Validate(); err != nil {
		return nil, errf.NewFromErr(errf.InvalidParameter, err)
	}
