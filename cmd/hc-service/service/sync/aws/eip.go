...
func (hd *eipHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if len(hd.eipList) == 0 {
		listOpt := &typeseip.AwsEipListOption{
			Region: hd.request.Region,
		}

		eipResult, err := hd.syncCli.CloudCli().ListEip(kt, listOpt)
		if err != nil {
			logs.Errorf("request adaptor list aws eip failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
			return nil, err
		}
		...
		hd.eipList = slice.Split(eipResult.Details, constant.CloudResourceSyncMaxLimit)
	}

	if len(hd.eipList) <= hd.offset {
		return nil, nil
	}

	cloudIDs := make([]string, 0, len(hd.eipList[hd.offset]))
	for _, one := range hd.eipList[hd.offset] {
...
func (hd *eipHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.Eip(kt, params, new(aws.SyncEipOption)); err != nil {
		logs.Errorf("sync aws eip failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
