...
func (svc *securityGroupSvc) BizListSGUsageBizMaintainers(cts *rest.Contexts) (interface{}, error) {
	...
	_, noPerm, err := handler.ListBizAuthRes(cts,
		&handler.ListAuthResOption{Authorizer: svc.authorizer, ResType: meta.Biz, Action: meta.Access})
	if err != nil {
		return nil, err
	}
	if noPerm {
	...
	securityGroups, err := svc.listSecurityGroupByIDs(cts.Kit, req.SecurityGroupIDs)
	if err != nil {
...
func buildSGUsageBizMaintainerResult(kit *kit.Kit, sgIDs []string, sgMap map[string]cloud.BaseSecurityGroup,
	...
	for _, sgID := range sgIDs {
		...
		for _, usageBizID := range sg.UsageBizIDs {
			...
			if !ok {
				logs.Errorf("business %d not found, rid: %s", usageBizID, kit.Rid)
				return nil, fmt.Errorf("business %d not found", usageBizID)
			}
			one.Details = append(one.Details, biz)
		}
...
func (svc *securityGroupSvc) searchBusinessByBizIDs(kit *kit.Kit, bizIDs []int64) (map[int64]cmdb.Biz, error) {
	...
	business, err := svc.esb.Cmdb().SearchBusiness(kit, param)
	if err != nil {
		logs.Errorf("search business failed, err: %v, rid: %s", err, kit.Rid)
		return nil, err
	}

	bizMap := make(map[int64]cmdb.Biz, len(business.Info))
