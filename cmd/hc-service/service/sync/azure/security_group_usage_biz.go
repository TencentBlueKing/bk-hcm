...
func (h *SGUsageBizRelSyncHandler) Sync(kt *kit.Kit) error {
	for {
		...
		if err != nil {
			logs.Errorf("list Azure security group for sync usage bizs failed, err: %v, rid: %s", err, kt.Rid)
			return err
		}
		if len(sgList) == 0 {
			break
		...
		if err := h.syncUsageBizs(kt, sgList); err != nil {
			logs.Errorf("sync Azure usage bizs failed, err: %v, rid: %s", err, kt.Rid)
			return err
		}
	}
	return nil
...
func (h *SGUsageBizRelSyncHandler) syncUsageBizs(kt *kit.Kit, sgList []corecloud.BaseSecurityGroup) error {
	params := &azure.SyncSGUsageBizParams{
