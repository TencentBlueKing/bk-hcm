...
func (svc *service) ListBillSummaryVersion(cts *rest.Contexts) (interface{}, error) {
	...
	data, err := svc.dao.AccountBillSummaryVersion().List(cts.Kit, opt)
	if err != nil {
		return nil, err
	}

	details := make([]*dataproto.BillSummaryVersionResult, len(data.Details))
	for indx, d := range data.Details {
		details[indx] = toProtoPullerResult(&d)
	}

	return &dataproto.BillSummaryVersionListResult{Details: details, Count: data.Count}, nil
}

func toProtoPullerResult(m *tablebill.AccountBillSummaryVersion) *dataproto.BillSummaryVersionResult {
	return &dataproto.BillSummaryVersionResult{
		ID:              m.ID,
