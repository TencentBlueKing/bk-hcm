...
- 该接口提供版本：v1.8.8.8。
- 该接口所需权限：无。
- 该接口功能描述：获取预测需要审批的 crp 单据。

### URL

POST /api/v1/woa/plans/resources/crp/ticket/list
    ...
    "tickets": [
      {
        ...
        "user": "admin",
        "approval_state": "hcm_admin_approval",
...
| url            | string | crp 链接                                                             |
| user           | string | 提单人                                                                |
| approval_state | string | 审批状态，"hcm_admin_approval"(hcm 管理员审批)、"crp_admin_approval"(crp 管理员审批) |
