...
- 该接口提供版本：v9.9.9+。
- 该接口所需权限：业务访问。
- 该接口功能描述：查询业务下权限策略库列表。

### URL

POST /api/v1/cloud/bizs/{bk_biz_id}/vendors/{vendor}/permission_policy_libraries/list
...
| bk_biz_ids               | int64 array | 允许使用的业务ID列表                                              |
| memo                     | string      | 描述                                                       |
| associated_account_count | int         | 关联二级账号数（仅统计管理业务为当前业务的账号） |
| creator                  | string      | 创建者                                                      |
| reviser                  | string      | 更新者                                                      |
| created_at               | string      | 创建时间，标准格式：2006-01-02T15:04:05Z                           |
| updated_at               | string      | 更新时间，标准格式：2006-01-02T15:04:05Z                           |
