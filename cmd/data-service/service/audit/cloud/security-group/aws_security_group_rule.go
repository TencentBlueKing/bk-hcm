...
func (s *SecurityGroup) awsSGRuleUpdateAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
	...
	idSgRuleMap, err := s.listAwsSGRule(kt, sg.ID, ids)
	if err != nil {
		return nil, err
	}
...
func (s *SecurityGroup) awsSGRuleDeleteAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
...
func (s *SecurityGroup) listAwsSGRule(kt *kit.Kit, sgID string, ids []string) (
	map[string]tablecloud.AwsSecurityGroupRuleTable, error) {
	...
	list, err := s.dao.AwsSGRule().List(kt, opt)
	if err != nil {
		logs.Errorf("list aws security group rule failed, err: %v, ids: %v, rid: %s", err, ids, kt.Rid)
		return nil, err
	}

	result := make(map[string]tablecloud.AwsSecurityGroupRuleTable, len(list.Details))
