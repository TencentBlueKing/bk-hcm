<template>
  <div class="recycle-manager-page">
    ...
    <bk-tab type="card-grid" v-model:active="selectedType">
      <template #setting>
        <div class="setting-icon-container" @click="() => (isSettingDialogShow = true)" v-show="currentAccountId">
...
export default defineComponent({
  ...
  setup() {
    ...
    // 从 route.query 获取 accountId，通过 accountSelectorStore 查账号详情
    const currentAccountId = computed(() => (route.query.accountId as string) || '');
    const currentAccount = computed(
      () =>
        accountSelectorStore.authorizedResourceAccountList.find(
          (a: { id: string }) => a.id === currentAccountId.value,
        ) || null,
    );
    ...
    const handleSettingConfirm = async () => {
      ...
      try {
        await http.patch(`${BK_HCM_AJAX_URL_PREFIX}/api/v1/cloud/accounts/${currentAccountId.value}`, {
    ...
    watch(
      ...
      (accountId) => {
        ...
        if (account?.recycle_reserve_time) {
          recycleReserveTime.value = account.recycle_reserve_time;
        }
      },
      {
        immediate: true,
