...
const logFileTag = "INFO"

const lineWrapStandard = " <------bigger context-----> "
...
type loggingT struct {
	...
	// file holds writer for the info log type.
...
func (l *loggingT) header(s severity, depth int) (*buffer, string, int) {
	...
	if !ok {
		...
		if length == 0 {
			...
			file = split[length-2] + "/" + split[length-1]
		}
	}
...
type syncBuffer struct {
	...
	*bufio.Writer
	file   *os.File
	nbytes uint32 // The number of bytes written to this file
}
...
func (sb *syncBuffer) Write(p []byte) (n int, err error) {
	if sb.nbytes+uint32(len(p)) >= MaxSize() {
...
func (sb *syncBuffer) rotateFile(now time.Time) error {
	if sb.file != nil {
		if err := sb.Flush(); err != nil {
			fmt.Fprintf(os.Stderr, "log: rotateFile flush: %v\n", err)
		}
		if err := sb.file.Close(); err != nil {
			fmt.Fprintf(os.Stderr, "log: rotateFile close: %v\n", err)
		}
	}
	var err error
	sb.file, _, sb.nbytes, err = create(now)
	if err != nil {
		return err
	}
...
// ErrorDepthf(0, "%s", "msg") is the same as Errorf("%s", "msg").
