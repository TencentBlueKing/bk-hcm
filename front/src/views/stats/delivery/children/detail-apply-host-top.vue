...
const fetchDetailData = async () => {
  ...
  const [currentRes, compareRes] = await Promise.all([
    http.post('/api/v1/woa/task/apply/findmany/bizs_hosts/statistics', {
  ...
  detailData.value = mergeTableData(currentData, compareData, ['host_count', 'order_count']);
    ...
    children: [
      {
        ...
        colKey: 'current_host_count',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.current_host_count - b.current_host_count,
        cell: (h, { row }) => row?.current_host_count ?? '--',
      },
      {
        title: compareMonth.value,
        colKey: 'compare_host_count',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.compare_host_count - b.compare_host_count,
        cell: (h, { row }) => row?.compare_host_count ?? '--',
      },
      {
        title: '环比',
        colKey: 'growth_rate',
        ...
        sortType: 'all',
        sorter: (a, b) => {
          const aValue = calculateGrowthRate(a.current_host_count, a.compare_host_count).value;
          const bValue = calculateGrowthRate(b.current_host_count, b.compare_host_count).value;
          return aValue - bValue;
        },
        cell: (h, { row }) => {
          const rate = calculateGrowthRate(row?.current_host_count, row?.compare_host_count);
          return h(
            'div',
            {
              class: rate?.class,
