...
func (cli *client) deleteSubnet(kt *kit.Kit, accountID string, region string, delCloudIDs []string) error {
	...
	if len(delFromCloud) > 0 {
		logs.Errorf("[%s] validate subnet not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloudZiyan, checkParams, len(delFromCloud), kt.Rid)
	...
	logs.Infof("[%s] sync subnet to delete subnet success, accountID: %s, count: %d, rid: %s",
...
func (cli *client) updateSubnet(kt *kit.Kit, accountID string, updateMap map[string]adtysubnet.TCloudSubnet) error {
	...
	if err := cli.dbCli.TCloudZiyan.Subnet.BatchUpdate(kt.Ctx, kt.Header(), updateReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch update db subnet failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync subnet to update subnet success, accountID: %s, count: %d, rid: %s",
...
func (cli *client) createSubnet(kt *kit.Kit, accountID string, region string,
	...
	if _, err := cli.dbCli.TCloudZiyan.Subnet.BatchCreate(kt.Ctx, kt.Header(), createReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch create subnet failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync subnet to create subnet success, accountID: %s, count: %d, rid: %s",
		enumor.TCloudZiyan, accountID, len(addSubnet), kt.Rid)
...
func (cli *client) listSubnetFromDB(kt *kit.Kit, params *SyncBaseParams) (
	...
	result, err := cli.dbCli.TCloudZiyan.Subnet.ListSubnetExt(kt.Ctx, kt.Header(), req)
	if err != nil {
		logs.Errorf("[%s] list subnet from db failed, err: %v, account: %s, req: %v, rid: %s",
...
func (cli *client) listSubnetFromCloud(kt *kit.Kit, params *SyncBaseParams) ([]adtysubnet.TCloudSubnet, error) {
	...
	if err != nil {
		logs.Errorf("[%s] list subnet from cloud failed, err: %v, account: %s, opt: %v, rid: %s",
...
func (cli *client) RemoveSubnetDeleteFromCloud(kt *kit.Kit, accountID string, region string) error {
	...
	for {
		...
		if err != nil {
			logs.Errorf("[%s] request dataservice to list subnet failed, err: %v, req: %v, rid: %s",
