...
type vpcHandler struct {
	...
	pager   *adazure.Pager[armnetwork.VirtualNetworksClientListResponse, types.AzureVpc]
}
...
func (hd *vpcHandler) Prepare(cts *rest.Contexts) error {
	...
	pager, err := hd.syncCli.CloudCli().ListVpcByPage(cts.Kit, listOpt)
	if err != nil {
		logs.Errorf("list vpc by page failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return err
	}
...
func (hd *vpcHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	for hd.pager.More() && len(total) < constant.CloudResourceSyncMaxLimit {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("list vpc next page failed, err: %v", err)
		}
...
func (hd *vpcHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	for _, partCloudIDs := range cloudIDElems {
		...
		if _, err := hd.syncCli.Vpc(kt, params, new(azure.SyncVpcOption)); err != nil {
			logs.Errorf("sync azure vpc failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
			return err
		}
	}
...
func (hd *vpcHandler) RemoveDeleteFromCloud(kt *kit.Kit) error {
	if err := hd.syncCli.RemoveVpcDeleteFromCloud(kt, hd.request.AccountID, hd.request.ResourceGroupName); err != nil {
		logs.Errorf("remove vpc delete from cloud failed, err: %v, accountID: %s, resGroupName: %s, rid: %s", err,
