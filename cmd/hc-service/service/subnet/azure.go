...
func (s subnet) AzureSubnetSync(cts *rest.Contexts) (interface{}, error) {
	...
	vpcInfo, isVpcExist, err := s.GetVpcInfoFromDBForAzure(cts, req, enumor.Azure)
	if err != nil {
		return nil, err
	}
...
func (s subnet) GetVpcInfoFromDBForAzure(cts *rest.Contexts, req *hcservice.ResourceSyncReq, vendor enumor.Vendor) (
	cloudcore.BaseVpc, bool, error) {
...
func (s subnet) BatchSyncAzureSubnetList(cts *rest.Contexts, req *hcservice.ResourceSyncReq,
	list *types.AzureSubnetListResult, resourceDBMap map[string]cloudcore.BaseSubnet) error {
	createResources, updateResources, existIDMap, err := s.filterAzureSubnetList(req, list, resourceDBMap)
	if err != nil {
		return err
	}
...
func (s subnet) batchCreateAzureSubnet(cts *rest.Contexts,
	...
	for i := 0; i < times; i++ {
		...
		if _, err := s.cs.DataService().Azure.Subnet.BatchCreate(cts.Kit.Ctx, cts.Kit.Header(), createReq); err != nil {
			return err
		}
	}
