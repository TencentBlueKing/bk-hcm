...
export const useMemoPagination = () => {
  ...
  watch(
    () => route.query,
    (query) => {
      if (query[PAGE_INDEX_KEY] && !isNaN(+query[PAGE_INDEX_KEY]))
        pageIndex.value = +query[PAGE_INDEX_KEY];
      if (query[PAGE_SIZE_KEY] && !isNaN(+query[PAGE_SIZE_KEY]))
        pageSize.value = +query[PAGE_SIZE_KEY];
    },
    {
      deep: true,
      immediate: true,
