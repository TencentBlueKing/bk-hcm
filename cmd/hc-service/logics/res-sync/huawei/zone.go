...
func (cli *client) Zone(kt *kit.Kit, opt *SyncZoneOption) (*SyncResult, error) {
	...
	// 只对 sync 的数据进行 diff
	addSlice, updateMap, delCloudIDs := common.Diff[typeszone.HuaWeiZone, corezone.BaseZone](
...
func (cli *client) createZone(kt *kit.Kit, opt *SyncZoneOption, addSlice []typeszone.HuaWeiZone) error {
	if len(addSlice) <= 0 {
		return errors.New("zone addSlice is <= 0, not create")
	}

	list := make([]datazone.ZoneBatchCreate[zone.HuaWeiZoneExtension], 0, len(addSlice))

	for _, one := range addSlice {
		zoneOne := datazone.ZoneBatchCreate[zone.HuaWeiZoneExtension]{
			CloudID: one.GetCloudID(),
			Name:    converter.PtrToVal(one.Code),
			State:   one.ResourceAvailability.Value(),
			Region:  opt.Region,
			NameCn:  converter.PtrToVal(one.Name),
			Source:  enumor.RegionSourceSync,
			Extension: &zone.HuaWeiZoneExtension{
				Port: converter.PtrToVal(one.Port),
			},
		}
		list = append(list, zoneOne)
	}

	createReq := &datazone.ZoneBatchCreateReq[zone.HuaWeiZoneExtension]{
		Zones: list,
	}
	_, err := cli.dbCli.HuaWei.Zone.BatchCreateZone(kt.Ctx, kt.Header(), createReq)
	if err != nil {
		logs.Errorf("[%s] create zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.HuaWei,
...
func (cli *client) updateZone(kt *kit.Kit, opt *SyncZoneOption,
	updateMap map[string]typeszone.HuaWeiZone) error {

	if len(updateMap) <= 0 {
		return errors.New("zone updateMap is <= 0, not update")
	}

	list := make([]datazone.ZoneBatchUpdate[zone.HuaWeiZoneExtension], 0, len(updateMap))

	for id, one := range updateMap {
		one := datazone.ZoneBatchUpdate[zone.HuaWeiZoneExtension]{
			ID:    id,
			State: one.ResourceAvailability.Value(),
		}
	...
	updateReq := &datazone.ZoneBatchUpdateReq[zone.HuaWeiZoneExtension]{
		Zones: list,
	}
	if err := cli.dbCli.HuaWei.Zone.BatchUpdateZone(kt.Ctx, kt.Header(), updateReq); err != nil {
		logs.Errorf("[%s] update zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.HuaWei,
			err, opt.AccountID, opt, kt.Rid)
		return err
	}
...
func (cli *client) findExistingZonesToDelete(addZones []typeszone.HuaWeiZone,
	allDBZones []corezone.BaseZone) []string {

	toDeleteCloudIDs := make([]string, 0)
	addZoneMap := converter.SliceToMap(addZones,
		func(t typeszone.HuaWeiZone) (string, interface{}) {
...
func (cli *client) deleteZone(kt *kit.Kit, opt *SyncZoneOption, delCloudIDs, toDeleteForAdd []string) error {
	...
	for _, one := range delZoneFromCloud {
		if _, exists := delCloudMap[one.GetCloudID()]; exists {
			logs.Errorf("[%s] validate zone not exist failed, before delete, opt: %v, exist zone id: %s, "+
				"del cloud ids: %v, rid: %s", enumor.HuaWei, opt, one.GetCloudID(), delCloudIDs, kt.Rid)
	...
	for _, parts := range elems {
		...
		if err != nil {
			logs.Errorf("[%s] delete zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.HuaWei,
				err, opt.AccountID, opt, kt.Rid)
			return err
		}
	}
...
func (cli *client) listZoneFromCloud(kt *kit.Kit, opt *SyncZoneOption) ([]typeszone.HuaWeiZone, error) {
	if err := opt.Validate(); err != nil {
		return nil, errf.NewFromErr(errf.InvalidParameter, err)
	}

	zoneOpt := &typeszone.HuaWeiZoneListOption{
		Region: opt.Region,
	}
	results, err := cli.cloudCli.ListZone(kt, zoneOpt)
	if err != nil {
		logs.Errorf("[%s] list zone from cloud failed, err: %v, account: %s, opt: %v, rid: %s", enumor.HuaWei,
			err, opt.AccountID, opt, kt.Rid)
		return nil, err
	}
...
func (cli *client) listZoneFromDB(kt *kit.Kit, opt *SyncZoneOption) ([]corezone.BaseZone, error) {
	...
	for {
		...
		if err != nil {
			logs.Errorf("[%s] list zone from db failed, err: %v, account: %s, req: %v, rid: %s", enumor.HuaWei, err,
...
func isZoneChange(cloud typeszone.HuaWeiZone, db corezone.BaseZone) bool {

	if cloud.ResourceAvailability.Value() != db.State {
		return true
	}
