...
对于需要触发**系统告警**的严重错误，在日志消息开头添加 `constant.SysAlarmFlag`。
...
const (
    ...
    SysAlarmFlag WarnSign = "SysAlarmFlag"
)

if err != nil {
    logs.Errorf("%s, call obs api failed, req: %+v, source: %s, schemeID: %s, err: %+v",
        constant.SysAlarmFlag, req, req.Source, ar.config.SchemeID, err)
    return nil, err
}

if r := recover(); r != nil {
    logs.Errorf("%s, cron job %s panic recovered after %d ms, panic: %v", constant.SysAlarmFlag, cj.Name, cost, r)
}

if err := cj.jobExecutor.GetRuntimeError(); err != nil {
    logs.Errorf("%s, cron job %s returns runtime err: %s", constant.SysAlarmFlag, cj.Name, err)
}
```
...
- 后续内容使用**逗号分隔**
- 包含足够的**上下文信息**便于排查
- 如果新增了`WarnSign`类型的常量，需要在bkmonitor告警平台配置对应日志关键字的告警
