...
type PollingHandler[T any, R any, Result any] interface {
	...
	Poll(kt *kit.Kit, client T, ids []*string) (R, error)
}
...
// PollUntilDone ...
func (poller *Poller[T, R, Result]) PollUntilDone(kt *kit.Kit, client T, ids []*string,
	...
	pollerFunc := func() (bool, *Result, error) {
		pollResult, err := poller.Handler.Poll(kt, client, ids)
		if err != nil {
