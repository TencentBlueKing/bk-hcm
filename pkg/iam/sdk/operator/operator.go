...
func (e *InOp) Match(match interface{}, with interface{}) (bool, error) {
	...
	for i := 0; i < valWith.Len(); i++ {
		hit, err := e.matchElement(match, valWith.Index(i).Interface())
		if err != nil {
			return false, err
		}
...
// matchElement match the in operator value's array element.
func (e *InOp) matchElement(match interface{}, with interface{}) (bool, error) {
