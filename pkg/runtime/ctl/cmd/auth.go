...
// authCmd disable/enable write Cmd.
type authCmd struct {
	cmd *Command
...
func (c *authCmd) GetCommand() *Command {
	return c.cmd
}

// Validate write related Command.
func (c *authCmd) Validate() error {
	if c.opt == nil {
...
// WithAuth init and returns the disable/enable/get write operations commands that auth server needed.
func WithAuth(opt *options.DisableWriteOption) []Cmd {
