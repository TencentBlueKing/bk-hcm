...
func (cli *client) createZone(kt *kit.Kit, opt *SyncZoneOption,
	...
	_, err := cli.dbCli.TCloud.Zone.BatchCreateZone(kt.Ctx, kt.Header(), createReq)
	if err != nil {
		logs.Errorf("[%s] create zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloud,
			err, opt.AccountID, opt, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync zone to create zone success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) updateZone(kt *kit.Kit, opt *SyncZoneOption,
	...
	if err := cli.dbCli.TCloud.Zone.BatchUpdateZone(kt.Ctx, kt.Header(), updateReq); err != nil {
		logs.Errorf("[%s] update zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloud,
			err, opt.AccountID, opt, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync zone to update zone success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) deleteZone(kt *kit.Kit, opt *SyncZoneOption, delCloudIDs []string) error {
	...
	for _, one := range delZoneFromCloud {
		if _, exsit := delCloudMap[converter.PtrToVal(one.ZoneId)]; exsit {
			logs.Errorf("[%s] validate zone not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
				enumor.TCloud, opt, len(delZoneFromCloud), kt.Rid)
	...
	for _, parts := range elems {
		...
		if err != nil {
			logs.Errorf("[%s] delete zone failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloud,
				err, opt.AccountID, opt, kt.Rid)
			return err
		}
	}

	logs.Infof("[%s] sync zone to delete zone success, accountID: %s, count: %d, rid: %s", enumor.TCloud,
...
func (cli *client) listZoneFromCloud(kt *kit.Kit, opt *SyncZoneOption) ([]typeszone.TCloudZone, error) {
	...
	if err != nil {
		logs.Errorf("[%s] list zone from cloud failed, err: %v, account: %s, opt: %v, rid: %s", enumor.TCloud,
			err, opt.AccountID, opt, kt.Rid)
		return nil, err
	}
...
func (cli *client) listZoneFromDB(kt *kit.Kit, opt *SyncZoneOption) (
	...
	for {
		...
		if err != nil {
			logs.Errorf("[%s] list zone from db failed, err: %v, account: %s, req: %v, rid: %s", enumor.TCloud, err,
