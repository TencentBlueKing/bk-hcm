...
func (svc *service) CreateBillSummaryVersion(cts *rest.Contexts) (interface{}, error) {
	...
	id, err := svc.dao.Txn().AutoTxn(cts.Kit, func(txn *sqlx.Tx, opt *orm.TxnOption) (interface{}, error) {
		summary := tablebill.AccountBillSummaryVersion{
		...
		if err != nil {
			return nil, fmt.Errorf("create account bill summary version failed, err: %v", err)
		}
		if len(ids) != 1 {
			return nil, fmt.Errorf("create account bill summary version expect 1 puller ID: %v", ids)
		}
		return ids[0], nil
	})
	if err != nil {
	...
	idStr, ok := id.(string)
	if !ok {
		return nil, fmt.Errorf("create account bill summary version but return id type not string, id type: %v",
