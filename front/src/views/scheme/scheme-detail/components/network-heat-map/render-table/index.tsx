        ...
        if (!area) {
          ...
          if (val.isCountry) {
            ...
            if (countryData && Array.isArray(countryData.children)) {
              cls = countryData.children.some(region => area.service_areas?.some(item => item.province_name === region.rowName)) ? 'highlight-cell' : 'no-highlight-cell';
            } else {
              cls = 'no-highlight-cell';
            }
          } else {
            cls = area.service_areas?.some(item => item.province_name === val.rowName) ? 'highlight-cell' : 'no-highlight-cell';
          }
        }
      }
      return (
                    ...
                    <tr>
                      <td class="tbody-col row-name-col">
                        <div class="cell country-cell">
                          <DownShape class={['arrow-icon', item.isFold ? 'fold' : '']} onClick={ () => { ctx.emit('toggleFold', item.rowName) } } />
