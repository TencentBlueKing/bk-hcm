...
// Check 联通性和云上字段匹配校验
func (a *AccountClient) Check(ctx context.Context, h http.Header, request *hsaccount.GcpAccountCheckReq) error {
...
func (a *AccountClient) GetRegionQuota(ctx context.Context, h http.Header,
	request *hsaccount.GetGcpAccountRegionQuotaReq) (*typeaccount.GcpProjectQuota, error) {

	resp := new(hsaccount.GetGcpAccountQuotaResp)
...
func (a *AccountClient) GetBySecret(ctx context.Context, h http.Header,
	request *cloud.GcpSecret) (*cloud.GcpInfoBySecret, error) {

	resp := new(account.BySecretResp[cloud.GcpInfoBySecret])
	err := a.client.Post().
		WithContext(ctx).
		Body(request).
