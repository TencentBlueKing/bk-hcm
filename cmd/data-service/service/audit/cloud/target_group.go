...
func (ad Audit) targetGroupAssignAuditBuild(kt *kit.Kit, assigns []protoaudit.CloudResourceAssignInfo) (
	...
	idMap, err := ad.listTargetGroup(kt, ids)
	if err != nil {
		return nil, err
	}
...
func (ad Audit) targetGroupDeleteAuditBuild(kt *kit.Kit, deletes []protoaudit.CloudResourceDeleteInfo) (
	[]*tableaudit.AuditTable, error) {
...
func (ad Audit) listTargetGroup(kt *kit.Kit, ids []string) (map[string]*tablelb.LoadBalancerTargetGroupTable, error) {
	opt := &types.ListOption{
		Filter: tools.ContainersExpression("id", ids),
		Page:   core.NewDefaultBasePage(),
	}
	list, err := ad.dao.LoadBalancerTargetGroup().List(kt, opt)
	if err != nil {
		logs.Errorf("list target group db failed, ids: %v, err: %v, rid: %s", ids, err, kt.Rid)
		return nil, err
	}
