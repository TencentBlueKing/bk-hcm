...
const fetchDetailData = async () => {
  ...
  const [currentRes, compareRes] = await Promise.all([
    http.post('/api/v1/woa/task/apply/delivery-rate/detail', {
  ...
  detailData.value = mergeTableData(currentData, compareData, ['host_delivery_rate', 'done_orders']);
...
const columns: Ref<TableProps['columns']> = computed(() => [
  ...
  {
    title: '需求交付率',
    colKey: 'host_delivery_rate',
    align: 'center',
    children: [
      {
        title: currentMonth.value,
        colKey: 'current_host_delivery_rate',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.current_host_delivery_rate - b.current_host_delivery_rate,
        cell: (h, { row }) =>
          row?.current_host_delivery_rate !== undefined ? `${row?.current_host_delivery_rate}%` : '--',
      },
      {
        title: compareMonth.value,
        colKey: 'compare_host_delivery_rate',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.compare_host_delivery_rate - b.compare_host_delivery_rate,
        cell: (h, { row }) =>
          row?.compare_host_delivery_rate !== undefined ? `${row?.compare_host_delivery_rate}%` : '--',
      },
      {
        title: '环比',
        colKey: 'growth_rate',
        ...
        sortType: 'all',
        sorter: (a, b) => {
          const aValue = calculateGrowthRate(a.current_host_delivery_rate, a.compare_host_delivery_rate).value;
          const bValue = calculateGrowthRate(b.current_host_delivery_rate, b.compare_host_delivery_rate).value;
          return aValue - bValue;
        },
        cell: (h, { row }) => {
          const rate = calculateGrowthRate(row?.current_host_delivery_rate, row?.compare_host_delivery_rate);
          return h(
            'div',
            {
              class: rate?.class,
