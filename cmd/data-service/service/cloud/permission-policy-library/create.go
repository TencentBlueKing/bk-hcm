...
// CreatePermissionPolicyLibrary batch create permission policy libraries.
func (svc *service) CreatePermissionPolicyLibrary(cts *rest.Contexts) (interface{}, error) {
	...
	switch vendor {
	case enumor.TCloud:
		return batchCreateForVendor(vendor, svc, cts)
	default:
		return nil, fmt.Errorf("unsupported vendor: %s", vendor)
	}
}

func batchCreateForVendor(vendor enumor.Vendor, svc *service, cts *rest.Contexts) (interface{}, error) {
	req := new(protocloud.PermissionPolicyLibraryBatchCreateReq)
	...
	result, err := svc.dao.Txn().AutoTxn(cts.Kit, func(txn *sqlx.Tx, opt *orm.TxnOption) (interface{}, error) {
		...
		for _, one := range req.PermissionPolicyLibraries {
			...
			models = append(models, tablecloud.PermissionPolicyLibraryTable{
				Name:           one.Name,
		...
		ids, err := svc.dao.PermissionPolicyLibrary().BatchCreateWithTx(cts.Kit, txn, models)
		if err != nil {
			logs.Errorf("batch create permission_policy_library failed, err: %v, rid: %s", err, cts.Kit.Rid)
			return nil, err
		}
