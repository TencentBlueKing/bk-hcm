...
// AzureRouteTableUpdate update azure route table.
func (r routeTable) AzureRouteTableUpdate(cts *rest.Contexts) (interface{}, error) {
	...
	getRes, err := r.cs.DataService().Azure.RouteTable.Get(cts.Kit.Ctx, cts.Kit.Header(), id)
	if err != nil {
		return nil, err
	}

	cli, err := r.ad.Azure(cts.Kit, getRes.AccountID)
	if err != nil {
		return nil, err
	}

	updateOpt := new(routetable.AzureRouteTableUpdateOption)
...
// AzureRouteTableDelete delete azure route table.
func (r routeTable) AzureRouteTableDelete(cts *rest.Contexts) (interface{}, error) {
	id := cts.PathParameter("id").String()
