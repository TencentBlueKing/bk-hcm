...
const fetchDetailData = async () => {
  ...
  const [currentRes, compareRes] = await Promise.all([
    http.post('/api/v1/woa/task/apply/completion-rate/detail', {
  ...
  detailData.value = mergeTableData(currentData, compareData, ['completion_rate', 'done_orders']);
    ...
    children: [
      {
        ...
        colKey: 'current_completion_rate',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.current_completion_rate - b.current_completion_rate,
        cell: (h, { row }) => (row?.current_completion_rate !== undefined ? `${row?.current_completion_rate}%` : '--'),
      },
      {
        title: compareMonth.value,
        colKey: 'compare_completion_rate',
        align: 'right',
        sortType: 'all',
        sorter: (a, b) => a.compare_completion_rate - b.compare_completion_rate,
        cell: (h, { row }) => (row?.compare_completion_rate !== undefined ? `${row?.compare_completion_rate}%` : '--'),
      },
      {
        title: '环比',
        colKey: 'growth_rate',
        ...
        sortType: 'all',
        sorter: (a, b) => {
          const aValue = calculateGrowthRate(a.current_completion_rate, a.compare_completion_rate).value;
          const bValue = calculateGrowthRate(b.current_completion_rate, b.compare_completion_rate).value;
          return aValue - bValue;
        },
        cell: (h, { row }) => {
          const rate = calculateGrowthRate(row?.current_completion_rate, row?.compare_completion_rate);
          return h(
            'div',
            {
              class: rate?.class,
