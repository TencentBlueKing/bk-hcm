...
- [x] 3.1 创建 `handlers/permission-template/update/init.go`：定义 `updatePermTemplateContent` 结构体，实现 `NewApplicationOfUpdatePermTemplate` 和 `newHandlerFromContent`，在 `init()` 中注册 `PermTemplateActionUpdate`
