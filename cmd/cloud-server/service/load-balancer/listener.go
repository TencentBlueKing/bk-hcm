...
// ListBizListenerByRsIP list biz listener by rsip.
func (svc *lbSvc) ListBizListenerByRsIP(cts *rest.Contexts) (any, error) {
	return svc.listListenerByRsIP(cts, handler.ListBizAuthRes)
}

func (svc *lbSvc) listListenerByRsIP(cts *rest.Contexts, authHandler handler.ListAuthResHandler) (any, error) {
	req := new(dataproto.ListListenerByRsIPReq)
	if err := cts.DecodeInto(req); err != nil {
		return nil, err
	}
	...
	if err != nil {
		logs.Errorf("list listener by rsip auth failed, noPermFlag: %v, err: %v, rid: %s", noPermFlag, err, cts.Kit.Rid)
		return nil, err
	}

	resList := &dataproto.ListListenerByRsIPResp{Details: make([]*dataproto.ListBatchListenerResult, 0)}
	if noPermFlag {
	...
	case enumor.TCloud:
		resList, err = svc.client.DataService().Global.LoadBalancer.ListLoadBalancerListenerByRsIP(cts.Kit, req)
		if err != nil {
			logs.Errorf("tcloud list listener by rsip failed, err: %v, req: %+v, rid: %s", err, req, cts.Kit.Rid)
			return nil, err
		}
		return resList, nil
	default:
