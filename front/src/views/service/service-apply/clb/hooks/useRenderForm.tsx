...
export default (formModel: Reactive<ApplyClbModel>) => {
        ...
        [
          {
            ...
            content: () => {
              const tooltips = {
                content: t('当前地域/可用区无可用的负载均衡规格类型'),
                disabled: hasAvailabilitySpecType.value,
                boundary: 'parent',
              };
              return (
                <Select
          ...
          },
          {
            ...
            content: () => {
              ...
              return (
                <Button disabled={!hasAvailabilitySpecType.value} onClick={handleClick}>
                  <Plus class='f24' />
                  {t('选择实例规格')}
                </Button>
              );
  ...
  const hasAvailabilitySpecType = computed(() => specAvailabilitySet.value.length > 0);
