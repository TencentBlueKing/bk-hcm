...
// SyncDisk ...
func SyncDisk(kt *kit.Kit, cliSet *client.ClientSet, accountID string, resourceGroupNames []string) error {
	...
	for _, name := range resourceGroupNames {
		...
		go func(name string) {
			...
			err := cliSet.HCService().Azure.Disk.SyncDisk(kt.Ctx, kt.Header(), req)
