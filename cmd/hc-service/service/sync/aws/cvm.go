...
func (hd *cvmHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.CvmWithRelRes(kt, params, new(aws.SyncCvmWithRelResOption)); err != nil {
		logs.Errorf("sync aws cvm failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
