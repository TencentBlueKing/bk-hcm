...
// ListListenerByRsIP list listener by rsip.
func (svc *lbSvc) ListListenerByRsIP(cts *rest.Contexts) (any, error) {
	req := new(protocloud.ListListenerByRsIPReq)
	if err := cts.DecodeInto(req); err != nil {
		return nil, err
	}
	...
	listenerList := &protocloud.ListListenerByRsIPResp{}
	for _, item := range req.ListenerQueryList {
		lblRsIPList, err := svc.queryListenerByRsIP(cts.Kit, req, item)
		if err != nil {
			return nil, err
		}
...
func (svc *lbSvc) queryListenerByRsIP(kt *kit.Kit, req *protocloud.ListListenerByRsIPReq,
	...
	if len(cloudClbIDs) == 0 {
		logs.Errorf("check list load balancer by rsip empty, req: %+v, rid: %s", cvt.PtrToVal(req), kt.Rid)
		return nil, nil
	}
	...
	if len(cloudLblIDs) == 0 {
		logs.Errorf("list biz listener by rsip empty, req: %+v, rid: %s", cvt.PtrToVal(req), kt.Rid)
		return nil, nil
	}

	// 根据RSIP获取绑定的目标组ID列表
	targetGroupRsList, targetGroupIDs, err := svc.listTargetByRsIP(kt, req, lblReq)
	if err != nil {
		return nil, err
	}
	// 未查询到符合的监听器列表
	if len(targetGroupIDs) == 0 {
		logs.Errorf("list load balancer target by rsip empty, req: %+v, rid: %s", cvt.PtrToVal(req), kt.Rid)
		return nil, nil
	}
	...
	return svc.convertListListenerByRsIP(lbMap, lblUrlRuleList, lblMap, targetGroupRsList)
}

func (svc *lbSvc) convertListListenerByRsIP(lbMap map[string]tablelb.LoadBalancerTable,
...
func (svc *lbSvc) listLoadBalancerListCheckVip(kt *kit.Kit, req *protocloud.ListListenerByRsIPReq,
...
func checkClbVipAndDomain(list *typeslb.ListLoadBalancerDetails, paramClbIDs, ipAddress []string) (
	...
	for idx, cloudID := range paramClbIDs {
		...
		// 检查对应的负载均衡VIP/域名是否匹配
		ip := ipAddress[idx]
		if cidr.IsDomainName(ip) && lbInfo.Domain != ip {
			return nil, nil, errf.Newf(errf.InvalidParameter, "load balancer[%s] domain is not match, "+
				"paramDomain: %s, clbDomain: %s", cloudID, ip, lbInfo.Domain)
		}

		switch lbInfo.LBType {
		case string(loadbalancer.InternalLoadBalancerType): // 内网
			if cidr.IsIPv4(ip) && !slice.IsItemInSlice(lbInfo.PrivateIPv4Addresses, ip) {
				return nil, nil, errf.Newf(errf.InvalidParameter, "load balancer[%s] privateIPv4 is not match, "+
					"paramIPv4: %s, clbPrivateIPv4: %v", cloudID, ip, lbInfo.PrivateIPv4Addresses)
			}
			if cidr.IsIPv6(ip) && !slice.IsItemInSlice(lbInfo.PrivateIPv6Addresses, ip) {
				return nil, nil, errf.Newf(errf.InvalidParameter, "load balancer[%s] privateIPv6 is not match, "+
					"paramIPv6: %s, clbPrivateIPv6: %v", cloudID, ip, lbInfo.PrivateIPv6Addresses)
			}
		case string(loadbalancer.OpenLoadBalancerType): // 公网
			if cidr.IsIPv4(ip) && !slice.IsItemInSlice(lbInfo.PublicIPv4Addresses, ip) {
				return nil, nil, errf.Newf(errf.InvalidParameter, "load balancer[%s] publicIPv4 is not match, "+
					"paramIPv4: %s, clbPublicIPv4: %v", cloudID, ip, lbInfo.PublicIPv4Addresses)
			}
			if cidr.IsIPv6(ip) && !slice.IsItemInSlice(lbInfo.PublicIPv6Addresses, ip) {
				return nil, nil, errf.Newf(errf.InvalidParameter, "load balancer[%s] publicIPv6 is not match, "+
					"paramIPv6: %s, clbPublicIPv6: %v", cloudID, ip, lbInfo.PublicIPv6Addresses)
			}
		default:
...
func (svc *lbSvc) listBizListenerByLbIDs(kt *kit.Kit, req *protocloud.ListListenerByRsIPReq,
	...
	for {
		...
		if err != nil {
			logs.Errorf("list biz listener by rsip failed, err: %v, req: %+v, rid: %s", err, cvt.PtrToVal(req), kt.Rid)
			return nil, nil, nil, fmt.Errorf("list biz listener by rsip failed, err: %v", err)
		}
...
// listTargetByRsIP 根据账号ID、RsIP查询绑定的目标组列表
func (svc *lbSvc) listTargetByRsIP(kt *kit.Kit, req *protocloud.ListListenerByRsIPReq,
...
func (svc *lbSvc) listTargetByCond(kt *kit.Kit, req *protocloud.ListListenerByRsIPReq,
	...
	for {
		...
		if err != nil {
			logs.Errorf("list load balancer target by rsip failed, err: %v, req: %+v, rid: %s",
				err, cvt.PtrToVal(req), kt.Rid)
			return nil, fmt.Errorf("list load balancer target by rsip failed, err: %v", err)
		}
