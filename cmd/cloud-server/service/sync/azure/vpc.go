...
// SyncVpc ...
func SyncVpc(kt *kit.Kit, cliSet *client.ClientSet, accountID string, resourceGroupNames []string,
	...
	defer func() {
		logs.V(3).Infof("azure account[%s] sync vpc end, cost: %v, rid: %s", accountID, time.Since(start), kt.Rid)
	}()
