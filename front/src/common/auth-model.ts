...
const getLoginUrl = () => {
  const cUrl = `${location.origin}/public/login_success.html?is_ajax=1`;
  if (/=%s/.test(process.env.BK_LOGIN_SERVICE_URL)) {
    return process.env.BK_LOGIN_SERVICE_URL.replace(/%s/, cUrl);
  }
  const loginUrl = new URL(process.env.BK_LOGIN_SERVICE_URL);
  if (/=$/.test(loginUrl.search)) {
    return process.env.BK_LOGIN_SERVICE_URL + cUrl;
  }
