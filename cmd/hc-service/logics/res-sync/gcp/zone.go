...
func (cli *client) createZone(kt *kit.Kit, opt *SyncZoneOption, addSlice []typeszone.GcpZone) error {
	if len(addSlice) <= 0 {
...
func (cli *client) updateZone(kt *kit.Kit, opt *SyncZoneOption, updateMap map[string]typeszone.GcpZone) error {
	if len(updateMap) <= 0 {
...
func (cli *client) findExistingZonesToDelete(addZones []typeszone.GcpZone, allDBZones []corezone.BaseZone) []string {
	toDeleteCloudIDs := make([]string, 0)
	addZoneMap := converter.SliceToMap(addZones,
		func(t typeszone.GcpZone) (string, interface{}) {
	...
	for _, one := range delZoneFromCloud {
		if _, exists := delCloudMap[one.GetCloudID()]; exists {
			logs.Errorf("[%s] validate zone not exist failed, before delete, opt: %v, exist zone id: %s, "+
				"del cloud ids: %v, rid: %s", enumor.Gcp, opt, one.GetCloudID(), delCloudIDs, kt.Rid)
