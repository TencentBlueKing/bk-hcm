...
- 该接口提供版本：v9.9.9+。
- 该接口所需权限：业务访问。
- 该接口功能描述：创建GPU需求提报主单，同时批量创建子单。前端通过excel导入预览接口获取解析数据后，将details回传至本接口完成主单和子单的创建。

### URL

POST /api/v1/woa/bizs/{bk_biz_id}/plans/resources/gpu/order/create
...
| op_product_name | string       | 是  | 运营产品名称                                 |
