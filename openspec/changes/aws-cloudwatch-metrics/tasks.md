...
- [ ] 2.1 `pkg/api/hc-service/instance-type/aws_cloudwatch.go`：更新 `AwsAssumeRoleGetMetricDataReq`（root_account_id + main_account_id + role_chain + region + metric_data_queries + start_time + end_time）
...
- [ ] 2.4 更新 `AwsAssumeRoleListMetricsReq`（root_account_id + main_account_id + role_chain + region + namespace + metric_name + dimensions）
- [x] 2.5 定义 `AwsAssumeRoleListMetricsResp`（Metrics 数组，每个包含 Namespace、MetricName、Dimensions）

## 3. hc-service CloudWatch handler

- [ ] 3.1 `cmd/hc-service/service/`：更新 GetMetricData handler（用 root_account_id 调 AwsRoot + main_account_id 查 CloudID → AwsWithAssumeRole → adaptor.GetMetricData）
- [ ] 3.2 更新 ListMetrics handler（同上模式 → adaptor.ListMetrics）
...
- [ ] 5.1 `cmd/cloud-server/service/`：更新 GetMetricData 资源视角 handler（移除 CloudID→AccountID 反查）
- [ ] 5.2 更新 ListMetrics 资源视角 handler
...
- [ ] 6.1 `docs/api-docs/web-server/docs/resource/get_aws_assume_role_metric_data.md`：更新 GetMetricData 接口文档
- [ ] 6.2 `docs/api-docs/web-server/docs/resource/list_aws_assume_role_metrics.md`：更新 ListMetrics 接口文档
