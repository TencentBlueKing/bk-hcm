...
// ListForHuaWei ...
func (i *instanceTypeAdaptor) ListForHuaWei(cts *rest.Contexts) (interface{}, error) {
	req := new(proto.HuaWeiInstanceTypeListReq)
	...
	client, err := i.adaptor.HuaWei(cts.Kit, req.AccountID)
	if err != nil {
		return nil, err
	}

	opt := &typesinstancetype.HuaWeiInstanceTypeListOption{
		Region: req.Region,
		Zone:   req.Zone,
	}
	...
	if err != nil {
		logs.Errorf("request adaptor to list huawei instance type failed, err: %v, opt: %v, rid: %s", err, opt,
			cts.Kit.Rid)
		return nil, err
	}
