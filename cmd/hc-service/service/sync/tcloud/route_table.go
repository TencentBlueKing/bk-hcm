...
func (hd *routeTableHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud routeTable failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
...
func (hd *routeTableHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.RouteTable(kt, params, new(tcloud.SyncRouteTableOption)); err != nil {
		logs.Errorf("sync tcloud routeTable failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
