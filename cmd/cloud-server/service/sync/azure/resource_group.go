...
func SyncResourceGroup(kt *kit.Kit, hcCli *hcservice.Client, accountID string) error {
	...
	defer func() {
		logs.V(3).Infof("azure account[%s] sync resource group end, cost: %v, rid: %s", accountID,
			time.Since(start), kt.Rid)
	}()

	req := &protohcregion.AzureRGSyncReq{
		AccountID: accountID,
	}
	if err := hcCli.Azure.ResourceGroup.SyncResourceGroup(kt.Ctx, kt.Header(), req); err != nil {
		logs.Errorf("sync azure region failed, err: %v, req: %v, rid: %s", err, req, kt.Rid)
		return err
	}
...
func ListResourceGroup(kt *kit.Kit, dataCli *dataservice.Client, accountID string) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("list azure resource group failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}

	if len(result.Details) == 0 {
		return nil, fmt.Errorf("account: %s resource group is empty", accountID)
	}

	resGroupName := make([]string, 0, len(result.Details))
	for _, one := range result.Details {
		resGroupName = append(resGroupName, one.Name)
	}

	return resGroupName, nil
}
