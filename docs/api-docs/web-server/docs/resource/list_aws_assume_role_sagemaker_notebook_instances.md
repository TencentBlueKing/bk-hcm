...
- 该接口提供版本：9.9.9。
- 该接口所需权限：资源查看。
- 该接口功能描述：查询 AWS 成员账号的 SageMaker Notebook Instance 列表。通过 STS AssumeRole 跨账号访问成员账号的 SageMaker 原生列表接口，透传返回 AWS 原始响应。

### URL

POST /api/v1/cloud/vendors/aws/assume_role/sagemaker/notebook_instances/list
...
| default_code_repository_contains | string | 否 | 默认代码仓库名称模糊匹配 |
...
| name_contains | string | 否 | Notebook Instance 名称模糊匹配 |
| next_token | string | 否 | AWS 分页 token |
| notebook_instance_lifecycle_config_name_contains | string | 否 | Lifecycle Config 名称模糊匹配 |
| sort_by | string | 否 | AWS 原生排序字段，如 Name、CreationTime、Status |
| sort_order | string | 否 | AWS 原生排序方向，如 Ascending、Descending |
| status_equals | string | 否 | AWS 原生状态过滤，如 InService、Stopped |
    ...
    "NotebookInstances": [
      {
        ...
        "LastModifiedTime": "2026-04-15T09:30:00Z",
...
| code | int | 状态码 |
| message | string | 请求信息 |
| data | object | AWS SageMaker `ListNotebookInstances` 原始响应透传，通常包含 `NotebookInstances` 和可选 `NextToken` |
| data.NotebookInstances | array | Notebook Instance 列表 |
