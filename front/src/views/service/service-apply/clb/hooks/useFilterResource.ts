...
import { BGP_VIP_ISP_TYPES, BGPVipIsp } from '@/constants';
...
export default (formModel: ApplyClbModel) => {
    ...
    ([map, backupZones]) => {
      ...
      if (!zones && !backupZones) {
        ...
        if (formModel.vendor === VendorEnum.ZIYAN) {
          ispList.value = handleResolveZiyanBGP(map.withoutZone?.ispList);
        }
      } else {
        ...
        if (formModel.vendor === VendorEnum.ZIYAN) {
          ispList.value = handleResolveZiyanBGP(map[key]?.ispList);
        }
      }
    },
    { deep: true },
  );
  // 自研云需要对BGP做拆分，TypeSet中的Type将作为实际入参值（tgw_group_name），不传vip_isp
  const handleResolveZiyanBGP = (list: IspEntry[] = []) => {
    return list.reduce((prev, curr) => {
      const { Isp, TypeSet } = curr;
      if (Isp === 'BGP') {
        prev.push(
          ...TypeSet.filter(({ Type }) => Type !== 'BGP').map(({ Type, Availability }) => ({
            Isp,
            TypeSet: [{ Type, Availability }],
          })),
        );
      } else {
        prev.push(curr);
  ...
  watch(
    ...
    (val) => {
      ...
      if (formModel.vendor === VendorEnum.ZIYAN) {
        formModel.vip_isp = val.length && availableBGP ? BGPVipIsp.ZIYAN_NORMAL_BGP : firstAvailableISP ?? '';
      }
    },
    { deep: true },
  );
